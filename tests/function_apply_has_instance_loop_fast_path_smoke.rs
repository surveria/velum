...
#[test]
fn batches_function_apply_has_instance_benchmark_loop() -> TestResult {
    ...
    let source = include_str!("corpora/benchmarks/active/function_apply_has_instance.js");
    let script = vm.compile(source)?;
    let before = vm.resource_usage();
    let value = vm.eval_compiled(&script)?;
    let after = vm.resource_usage();
    ensure_value(&value, &Value::Number(806_404_560_000.0))?;
    ...
    if direct_runs < 480_000 {
        return Err(format!("expected at least 480000 direct runs, got {direct_runs}").into());
    }
    Ok(())
}
