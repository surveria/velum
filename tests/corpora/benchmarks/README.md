...
The default `full` set preserves all project cases. `RSQJS_BENCH_SET=sentinel` runs the small prepared merge-to-merge set. Filters are comma-separated exact ids; use a trailing `*` only when a prefix is intentional.

Prepared sources under `prepared/` must define three global functions:

- `__rsqjsBenchSetup()` builds reusable state outside the measured interval.
- `__rsqjsBenchRun()` performs the thematic operation and returns the same primitive checksum on every invocation.
- `__rsqjsBenchVerify()` returns the known checksum without repeating the workload.
...
When a prepared source, protocol, sampling configuration, reference engine, or benchmark host changes, refresh and review `quickjs-baseline.tsv` with `RSQJS_QUICKJS_BASELINE=refresh`. Read mode never reuses a partial key match.
