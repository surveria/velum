...
fn inherited_work_tree_cannot_hide_modified_supplied_corpus() -> TestResult {
    with_fixture(|fixture| {
        ...
        let output = fixture.launch(&[("GIT_WORK_TREE", fixture.pristine.clone().into_os_string())])?;
        fixture.reject_before_runner(&output, "GIT_WORK_TREE")?;
        require(fixture.protected_snapshot()? == before, "Git isolation changed protected metadata")
    })
}
...
fn inherited_git_context_never_redirects_validation_to_pristine_clone() -> TestResult {
    with_fixture(|fixture| {
        ...
        for (name, overrides) in fixture.redirecting_overrides() {
            ...
            require(fixture.protected_snapshot()? == before, &format!("{name} changed protected metadata"))?;
        }
        Ok(())
    })
}
...
fn clean_corpus_runs_with_sanitized_git_environment_and_preserved_sentinels() -> TestResult {
    with_fixture(|fixture| {
        ...
        for (name, overrides) in fixture.all_overrides() {
            ...
            require(fixture.root.join("executed.txt").is_file(), "clean fixture never reached fake PGO")?;
            fixture.check_child_git_environment()?;
            require(fixture.protected_snapshot()? == before, &format!("{name} changed corpus, pristine clone, or external Git storage"))?;
...
impl Fixture {
    ...
    fn prepare(&self) -> TestResult {
        ...
        fs::write(self.corpus.join("test/case.js"), "// Upstream test fixture.\n")?;
        fs::write(self.corpus.join("harness/assert.js"), "// Upstream harness fixture.\n")?;
        self.git(&self.corpus, &["init", "--quiet"])?;
        self.git(&self.corpus, &["add", "test", "harness"])?;
        self.git(&self.corpus, &[
            "-c", "user.name=Git isolation fixture", "-c", "user.email=fixture@example.invalid",
            "-c", "commit.gpgsign=false", "-c", "core.hooksPath=/dev/null",
            "commit", "--quiet", "-m", "Create tiny local corpus",
        ])?;
        let pin = self.git_text(&self.corpus, &["rev-parse", "HEAD"])?;
        let tree = self.git_text(&self.corpus, &["rev-parse", "HEAD^{tree}"])?;
        self.clone_pristine()?;
        ...
        for corpus in [&self.corpus, &self.pristine] {
            ...
            fs::write(corpus.join("harness/assert.js"), "// Patched harness fixture.\n")?;
        }
        fs::write(self.root.join("external-index"), "external index sentinel: do not overwrite\n")?;
        for directory in ["external-objects", "external-alternates"] {
            fs::create_dir(self.root.join(directory))?;
            fs::write(self.root.join(directory).join("sentinel"), "external Git objects must not change\n")?;
        }
    ...
    fn clone_pristine(&self) -> TestResult {
        let output = self.command("git").args(["clone", "--quiet", "--no-hardlinks"])
            .arg(&self.corpus).arg(&self.pristine).output()?;
    ...
    fn prepare_experiment(&self, pin: &str, tree: &str) -> TestResult {
        ...
        fs::write(test262.join("full-pass-baseline.txt"), format!(
        ...
        fs::write(self.experiment.join("provenance.txt"), format!("commit={pin}\ntree={tree}\n"))?;
        fs::write(self.experiment.join("result.txt"), "status=complete-needs-review\nexit_code=0\n")?;
        for (name, script) in [("pgo", MOCK_PGO), ("ordinary", MOCK_VERIFIER)] {
            ...
            self.hash_files(&self.experiment.join("bin").join(format!("{name}.sha256")), &[binary], &self.root)?;
        }
        ...
        self.hash_files(&self.experiment.join("profile.sha256"), &[profile], &self.root)?;
        let files = [
        ...
        self.hash_files(&self.experiment.join("source-files.sha256"), &files, &source)?;
        let layout = self.command("find").current_dir(&source)
            .args([".", "-printf", "%y\t%p\t%l\n"]).output()?;
        ...
        lines.sort_unstable();
        fs::write(self.experiment.join("source-layout.tsv"), format!("{}\n", lines.join("\n")))?;
        Ok(())
    }

    fn command(&self, program: &str) -> Command {
        let mut command = Command::new(program);
        command.env_clear().env("PATH", &self.system_path).env("HOME", &self.root)
            .env("LC_ALL", "C").env("GIT_CONFIG_NOSYSTEM", "1")
            .env("GIT_CONFIG_GLOBAL", "/dev/null");
        command
    }

    fn git(&self, directory: &Path, arguments: &[&str]) -> TestResult {
        successful(&self.command("git").current_dir(directory).args(arguments).output()?, "temporary Git fixture")
    }

    fn git_text(&self, directory: &Path, arguments: &[&str]) -> Fallible<String> {
        let output = self.command("git").current_dir(directory).args(arguments).output()?;
    ...
    fn hash_files(&self, output: &Path, files: &[PathBuf], directory: &Path) -> TestResult {
        let result = self.command("sha256sum").current_dir(directory).arg("--").args(files).output()?;
    ...
    fn launch(&self, overrides: &[(&str, OsString)]) -> Fallible<Output> {
        ...
        command.args(["--kill-after=1s", "20s", "bash"]).arg(self.root.join("launcher.sh"))
            .arg(&self.experiment).arg(&self.corpus).arg(self.root.join("qjs"))
            .env("MOCK_PGO_GIT_ROOT", &self.root).stdin(Stdio::null());
    ...
    fn modify_supplied_corpus(&self) -> TestResult {
        fs::write(self.corpus.join("test/case.js"), "// Modified supplied corpus must fail.\n")?;
        Ok(())
    }
    ...
    fn redirecting_overrides(&self) -> Vec<(&'static str, Overrides)> {
        ...
        vec![
            ("GIT_DIR", vec![("GIT_DIR", git_dir.clone()), ("GIT_WORK_TREE", worktree.clone())]),
            ("GIT_COMMON_DIR", vec![("GIT_COMMON_DIR", git_dir), ("GIT_WORK_TREE", worktree.clone())]),
            ("GIT_CONFIG_COUNT", vec![
                ("GIT_CONFIG_COUNT", "1".into()), ("GIT_CONFIG_KEY_0", "core.worktree".into()),
                ("GIT_CONFIG_VALUE_0", worktree),
            ]),
            ("GIT_CONFIG_PARAMETERS", vec![("GIT_CONFIG_PARAMETERS",
    ...
    fn all_overrides(&self) -> Vec<(&'static str, Overrides)> {
        ...
        cases.extend([
            ("GIT_WORK_TREE", vec![("GIT_WORK_TREE", self.pristine.clone().into_os_string())]),
            ("standalone GIT_DIR", vec![("GIT_DIR", self.root.join("missing-git-directory").into_os_string())]),
            ("standalone GIT_COMMON_DIR", vec![("GIT_COMMON_DIR", self.root.join("missing-common-directory").into_os_string())]),
            ("GIT_INDEX_FILE", vec![("GIT_INDEX_FILE", self.root.join("external-index").into_os_string())]),
            ("GIT_OBJECT_DIRECTORY", vec![
                ("GIT_OBJECT_DIRECTORY", self.root.join("external-objects").into_os_string()),
                ("GIT_ALTERNATE_OBJECT_DIRECTORIES", self.root.join("external-alternates").into_os_string()),
            ]),
            ("GIT_PREFIX", vec![("GIT_PREFIX", "test/".into())]),
            ("unknown exported Git override", vec![("GIT_FUTURE_OVERRIDE_FIXTURE", "must-be-cleared".into())]),
        ]);
        cases
    }

    fn reject_before_runner(&self, output: &Output, name: &str) -> TestResult {
        require(output.status.code() == Some(2), &format!("{name}: expected corpus rejection, got {:?}: {}", output.status, String::from_utf8_lossy(&output.stderr)))?;
        require(String::from_utf8_lossy(&output.stderr).contains("Test262 content differs from the exact pinned commit plus two tracked patches"), &format!("{name}: rejected for an unrelated reason"))?;
        require(!self.root.join("executed.txt").exists(), &format!("{name}: modified corpus reached the fake PGO runner"))
    }

    fn protected_snapshot(&self) -> Fallible<Snapshot> {
        let mut snapshot = BTreeMap::new();
        for directory in [self.corpus.join(".git"), self.pristine.join(".git"), self.root.join("external-objects"), self.root.join("external-alternates")] {
            collect_files(&directory, &mut snapshot)?;
        }
        let index = self.root.join("external-index");
        require(snapshot.insert(index.clone(), fs::read(index)?).is_none(), "duplicate external index snapshot")?;
        Ok(snapshot)
    }

    fn check_child_git_environment(&self) -> TestResult {
        let text = fs::read_to_string(self.root.join("child-environment.txt"))?;
        let actual: Vec<_> = text.lines().filter(|line| line.starts_with("GIT_")).collect();
        let expected = ["GIT_CONFIG_GLOBAL=/dev/null", "GIT_CONFIG_NOSYSTEM=1", "GIT_NO_REPLACE_OBJECTS=1", "GIT_OPTIONAL_LOCKS=0"];
        require(actual == expected, &format!("fake PGO inherited unexpected Git context: {actual:?}"))
    }
}
...
fn collect_files(directory: &Path, snapshot: &mut Snapshot) -> TestResult {
    for entry in fs::read_dir(directory)? {
        ...
        if entry.file_type()?.is_dir() {
            ...
            require(entry.file_type()?.is_file(), "unexpected symlink in tiny Git fixture")?;
            require(snapshot.insert(path.clone(), fs::read(path)?).is_none(), "duplicate protected Git path")?;
        }
    }
    Ok(())
}
...
fn with_fixture(test: impl FnOnce(&Fixture) -> TestResult) -> TestResult {
    ...
    match (result, cleanup) {
        ...
        (Err(error), Err(cleanup)) => Err(format!("{error}; fixture cleanup failed: {cleanup}").into()),
    }
}
