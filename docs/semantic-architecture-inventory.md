...
| `[[GetPrototypeOf]]` | `semantic_get_prototype` | ObjectHeap/function/error prototype owners remain physical backends; derived class constructors expose their static parent and HostFunction retains its realm-correct Function prototype as a traced value | AS-02b2 complete in PR #403; static class inheritance aligned by AS-09c; host backend completed by the callable-as-object audit tranche |
| `[[SetPrototypeOf]]` | `semantic_try_set_prototype` | Function and HostFunction stores retain Value prototypes with extensibility and cycle checks; ObjectHeap still stores only `ObjectId` prototypes, so callable-valued prototypes for ordinary objects remain unsupported storage debt | AS-02b2 complete in PR #403; AS-05 owns the remaining ObjectHeap handle/storage redesign |
...
| Dynamic compilation | one `dynamic_compilation_error` owner converts only eval and Function/AsyncFunction lexer/parser failures to typed `SyntaxError` requests while retaining the dynamic source span; `CompiledScript` retains the parser-derived strictness bit so eval selects its lexical/variable environment shape without reparsing; sloppy direct eval gives lexical declarations one disposable compiled scope while typed eval-variable operands hoist vars into the caller function VariableEnvironment, and closures retain a traced deletable eval binding environment; strict eval keeps both declaration classes local, indirect eval adds an explicit global activation boundary, and configurable global eval declarations remain synchronized with global-object deletion; runtime, resource, and existing JavaScript errors remain unchanged | AS-09b merged in PR #451 as `b11ce20f`; PR #484 extends the same owner with eval environment selection; PR #630 replaces active-scope inference with explicit eval environment ownership |
...
| direct eval variable environments | one typed `EvalVariable` binding-layout operand distinguishes eval vars from lexical globals; sloppy functions record their own direct-eval capability, call activations retain a stable function id while continuations are parked, eval-created closures capture one traced and storage-counted deletable binding environment rooted in the caller VariableEnvironment, and pre-setup global loads bypass caller-local dynamic environments while ordinary execution retains Annex B catch shadowing | treating eval vars as globals, selecting the top lexical scope as VariableEnvironment, consulting caller activations for an unrelated function, resolving a pre-setup global load through a caller-local scope, losing eval cells from tracing/accounting, or retaining deleted eval bindings as declarative bindings |
