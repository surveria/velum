...
| async host command boundary | an async host-call frame can copy a VM-bound sender into its Rust future; the sender transfers one source-proven retained callable and owned primitive arguments into a limited, accounted VM-local FIFO without borrowing or reentering the VM; explicit embedder dispatch delegates to the shared semantic call owner, adopts synchronous and asynchronous results through the existing Promise owner, and an ordinary Promise reaction returns an owned fulfillment or the exactly retained JavaScript rejection to the waiting Rust future; dispatch, Promise jobs, and Rust-future polling remain three explicit phases with cancellation and teardown boundaries | generated source or eval, holding a mutable VM borrow across await, a parallel JavaScript call implementation, bypassing Promise adoption, implicit job/future driving, an unrooted callable or rejection, a foreign-VM handle, a wake while the command mutex is held, or command storage/captures that survive abandonment or cancellation |
