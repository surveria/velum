...
| Frontend source and token ownership | well-formed source retains one owned UTF-8 rendering, while dynamic compilation of an ill-formed ECMAScript string adds only a sparse byte-offset map for its lone UTF-16 surrogates; checked UTF-8 cursor boundaries remain the diagnostic coordinate owner, and string, template, and RegExp tokens recover exact code units before immutable shared static-value interning; speculative replay and parser consumption reuse those payloads, `CompiledScript` discards the complete source, and each ordinary function record retains only its exact source slice for `Function.prototype.toString`; the `SourceRecord` ledger reserves count and payload before function insertion, replaces generated display source transactionally, and releases both through GC reconciliation; cursor, smart-pointer, token-buffer, sparse-map, and interning representations may change as long as exact UTF-16 and function-source semantics remain observable without restoring redundant complete-source ownership | PR #562 removes the audit allocation copies; PR #631 extends exact ill-formed dynamic source; PR #632 adds exact ordinary-function source ownership |
