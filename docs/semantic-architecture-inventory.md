...
| direct eval variable environments | one typed `EvalVariable` binding-layout operand distinguishes eval vars from lexical globals; sloppy functions record their own direct-eval capability, call activations distinguish captured from active dynamic environments, and resolution orders eval-local scopes, active dynamic environments, caller function locals, then captured dynamic environments; eval-created closures retain traced captured lexical cells plus one storage-counted deletable binding environment rooted in the caller VariableEnvironment without duplicating those bindings as captured declarative cells; lexical arrows preserve the direct-eval `new.target` permission, and pre-setup global loads bypass caller-local dynamic environments while ordinary execution retains Annex B catch shadowing | treating eval vars as globals, selecting the top lexical scope as VariableEnvironment, consulting caller activations for an unrelated function, resolving a pre-setup global load through a caller-local scope, losing eval cells from tracing/accounting, duplicating eval-owned cells in a captured declarative scope, or retaining deleted eval bindings as declarative bindings |
