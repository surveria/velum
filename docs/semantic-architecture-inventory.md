...
| AS-09af | BigInt buffer-view content ownership | Draft PR #513 extends the existing typed-array element-kind owner with `BigInt64Array` and `BigUint64Array`, exact modulo-2^64 signed/unsigned writes, and `Value`-typed reads. One Number/BigInt content discriminator selects `ToNumber` or `ToBigInt` before storage, rejects cross-content construction/species results, and leaves Number fast paths on the same byte owner. DataView reuses the same discriminator for four endian-aware BigInt accessors. The exact direct profile advances from 22/194 to 170/194 variants; its residuals require the separately owned detach harness or immutable ArrayBuffer API. Shared integer-indexed descriptors, own-key enumeration, conversion order, `fill`, and `%TypedArray%.from` handling raise the broad TypedArray/DataView profile by 1,151 variants to 4,264/5,446 while all active and QuickJS differential cases remain green. SharedArrayBuffer and Atomics remain later synchronization tranches. |
