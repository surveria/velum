...
| AS-09y | shared ArrayBuffer lifecycle ownership | Draft PR #492 replaces the fixed byte-vector owner with one shared resizable/detachable state used by ArrayBuffer, TypedArray, and DataView clones. Branded accessors, `isView`, resize, species-aware slice, and both transfer modes preserve one physical byte owner and update ObjectHeap payload accounting on mutation. The exact focused profile advances from 56/442 to 320/442 variants and from 28/221 to 160/221 files; residuals are isolated to immutable proposal methods, SharedArrayBuffer, `$262`, BigInt, allocation ceilings, and the native-subclass/newTarget boundary. |
