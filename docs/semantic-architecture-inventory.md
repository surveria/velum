...
| AS-09v | `Promise.prototype.finally` species and reaction capability ownership | Draft PR #485 routes Finally through shared species construction, generic capabilities, observable `then`, and thenable jobs. One traced native-function state category retains Then/Catch Finally handlers and value-thunk/thrower settlements; Promise reactions settle either intrinsic ids or generic resolve/reject values. Observable Promise adoption and `catch` no longer bypass `then`. The exact Finally profile advances from 4/58 to 58/58 variants and from 2/29 to 29/29 files, while four dependent combinator variants also pass. |
