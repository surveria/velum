...
| RegExp | `regexp_value` on `Object` owns one immutable, `Rc`-shared state containing exact UTF-16 source units, parsed flags, and the compiled in-tree `regress` program; `RegExp.prototype.compile` transactionally replaces that whole state | RegExp built-ins plus ordinary properties; construction compiles once and charges source work once, repeated matching reuses the VM-local program and charges the scanned input instead of the immutable source again, retained program storage is charged to the owning object's payload, and no compiled state crosses VM boundaries |
| Proxy | `proxy_value` on `Object` | repeated pre-dispatch in get/has/set/delete/prototype/descriptor/call/construct paths |
| ArrayBuffer / SharedArrayBuffer | `byte_buffer` on `Object`; AS-09ag keeps ordinary buffers in `Rc<RefCell<ByteBufferState>>` and explicitly shared buffers in `Arc<parking_lot::RwLock<ByteBufferState>>`, with both storage kinds exposed through one view API | branded buffer accessors/methods plus ordinary properties; local resize/detach and shared grow update the ObjectHeap byte payload total, while Atomics and TypedArray bulk operations hold one bounded read or exclusive-write guard across their complete physical storage operation |
| TypedArray | `typed_array` on `Object`, carrying a Number- or BigInt-content element kind and referencing a buffer object plus shared byte buffer | typed-array indexed branches plus ordinary properties; one element-kind owner performs exact Number/BigInt encoding and decoding for scalar and bulk paths, while fill, set, sort write-back, slice, copyWithin, and reverse borrow their backing byte view a bounded number of times and preserve snapshot or sequential aliasing according to the semantic operation |
...
| Map/Set/WeakMap/WeakSet | `collections: Vec<CollectionData>` whose backing store retains `CollectionKind`; one immutable SameValueZero key index owns average constant-time lookup while a separate ordered entry log owns iteration order | `collection_object_slots: Vec<Option<(CollectionKind, CollectionId)>>` | Map/Set slots trace strongly; WeakSet keys trace weakly; WeakMap key/value pairs trace as ephemerons; weak identity accepts ordinary objects, JavaScript/native/host functions, and non-registered Symbols through their existing typed reachability categories; deletion history compacts transactionally when no registered iterator or callback cursor pins it, and the index/log representation may change while lookup complexity, ordering, mutation visibility, tracing, and ledger counts remain intact |
| Collection and RegExp iterators | `collection_iterators: Vec<CollectionIteratorState>` | iterator id captured by a native `next` function | Map/Set iterators retain live ordered-log cursors and pin deletion history until completion; snapshot and RegExp iterator states retain their explicit strong `Value` edges |
...
kind allowlist. Bound-function creation also copies the target's semantic
`[[Prototype]]`, including callable and null values, while construction retains
the target replacement rule for `newTarget`.
...
| `[[Get]]` | `Context::semantic_property_read[_with_receiver]` plus `finish_semantic_property_read` | AS-02b1 owns object-like dispatch; the AS-03b2 `Get` operation owns specification composition, while static caches optimize only the returned ordinary-object tail; virtual global bindings synthesize own values only and leave prototype traversal, including Proxy receivers, to this owner | AS-02b1 complete after PR #401; PR #643 removes the physical global-prototype bypass |
| `[[HasProperty]]` | `Context::semantic_property_presence` plus `finish_semantic_property_presence` | AS-02b1 now owns object-like dispatch; static presence caches optimize only the returned ordinary-object tail, while primitive rejection remains in the property layer; an absent virtual global fact returns an ordinary-object tail instead of suppressing semantic prototype traversal | AS-02b1 complete after PR #401; AS-03b later owns the abstract operation; PR #643 aligns globals |
...
| `[[DefineOwnProperty]]` | `semantic_define_own_property_*` | the semantic owner performs compatibility and extensibility validation before dispatching to ObjectHeap or callable physical stores, preserving the specification boolean result; JavaScript function stores accept shared data/accessor updates, while native functions remain a data-only physical backend | AS-02b2 complete in PR #403; JavaScript function accessors added by AS-09c; PR #643 aligns callable false-return behavior |
...
| `[[GetOwnProperty]]` | `semantic_own_property_descriptor` | Object/string/global/function physical descriptor stores remain backends; JavaScript, native, and host function variants share `FunctionProperties` descriptor semantics | AS-02b2 complete in PR #403; host backend completed by the callable-as-object audit tranche |
| `[[GetPrototypeOf]]` | `semantic_get_prototype` | ObjectHeap and all callable stores retain semantic `Value` prototypes; derived class constructors expose their static parent, bound functions copy their target's prototype, and HostFunction retains its realm-correct Function prototype as a traced value | AS-02b2 complete in PR #403; callable-valued ordinary prototypes and bound inheritance aligned by PR #632 |
| `[[SetPrototypeOf]]` | `semantic_try_set_prototype` | ObjectHeap, Function, and HostFunction stores retain semantic `Value` prototypes with extensibility and cycle checks; the ordinary cycle walk follows ordinary and callable prototypes but stops at a Proxy exotic boundary without invoking its observable `[[GetPrototypeOf]]` trap | AS-02b2 complete in PR #403; callable-valued ordinary prototype storage completed by PR #632 |
| extensibility/integrity | `semantic_{is,prevent}_extensions` and semantic integrity-level methods | Ordinary objects use ObjectHeap directly; Proxy integrity composes traps and descriptors; JavaScript, native, and host function stores retain explicit extensibility, sealed, and frozen state | AS-02b2 complete in PR #403; host backend completed by the callable-as-object audit tranche |
...
| Dynamic compilation | one `dynamic_compilation_error` owner converts only eval and Function/AsyncFunction lexer/parser failures to typed `SyntaxError` requests while retaining the dynamic source span; `CompiledScript` retains parser-derived execution facts so eval selects its lexical/variable environment shape without reparsing; each sloppy function activation that can perform direct eval creates one live eval binding environment before nested functions capture it, lexical declarations use one disposable compiled scope, and typed eval-variable operands append deletable bindings rooted in the caller Function VariableEnvironment; strict eval keeps both declaration classes local, indirect eval adds an explicit global activation boundary, lexical arrows preserve direct-eval `new.target` permission, and configurable global eval declarations remain synchronized with global-object deletion; runtime, resource, and existing JavaScript errors remain unchanged | AS-09b merged in PR #451 as `b11ce20f`; PR #484 extends the same owner with eval environment selection; PR #630 replaces active-scope inference with explicit eval environment ownership; PR #632 adds captured lexical and lexical `new.target` ownership |
...
| Frontend `await` grammar ownership | host Script compilation, ECMAScript eval Script parsing, Module parsing, function parameters and bodies, and class-field initializers each establish and restore their own expression-allowed and identifier-reserved context; the host-only top-level-await extension never leaks into eval, Module reservation survives nested ordinary functions, and class-field initializers do not inherit an enclosing async function's await expressions; parser state and helper layout may change while behavioral nested-context coverage preserves the grammar contract | PR #643 replaces ambient parser defaults at eval, function, and class-field boundaries without adding a helper-spelling guard |
...
| Frontend function `arguments` ownership | each ordinary function parses parameters and body in one isolated usage context; nested ordinary functions own their references independently, while arrows inherit the nearest ordinary owner; FunctionDeclarationInstantiation creates at most one implicit binding, and a same-name `var` declaration reuses it rather than installing an undefined shadow; the parser state and binding-allocation representation may change as long as these ownership semantics and lazy allocation remain intact | PR #559 replaces the parser-global monotonic reference counter; PR #632 aligns same-name var instantiation |
| Function parameter initialization | every parameter retains its binding or pattern target through AST and bytecode; one call-time initializer creates all parameter bindings first, then processes each argument, whole-target default, and destructuring target strictly left-to-right before advancing to the next parameter or hoisting the body; the AST, bytecode, scope, and walker representations may change as long as earlier bindings are observable to later defaults, later bindings retain TDZ, iterator closing and abrupt completion remain shared, and parameter patterns never become body declarations | PR #573 removes parser-generated `%patternN%`/`%rest%` names and body prologues, reuses the typed pattern walker in parameter mode, and adds direct ordering, TDZ, and rest-position coverage without adding a representation guard |
| JSON text semantics | `JSON.parse`, RawJSON validation, and `QuoteJSONString` use engine-owned ECMAScript grammar over exact UTF-16 code units; numeric text converts directly to binary64, lone surrogates in string values remain code units or are emitted as well-formed escapes, nesting follows the VM expression-depth limit, and parsed child values remain transiently rooted during materialization | PR #574 removes the root engine's `serde_json` dependency and adds focused number, surrogate, grammar, depth, boxed-string conversion, RawJSON, active-corpus, and differential evidence. Parser modules, tree representation, and quoting helpers may be replaced without changing this semantic contract; no source-layout guard is added |
| Frontend syntax preservation | exponentiation rejects every unparenthesized unary-form left operand including `await`; `debugger` retains statement identity and source span through the frontend while compiling as a no-op until an inspector contract exists; dynamic-import constructor forms are syntax errors without entering the binding namespace, while `new import.meta()` in Module code remains an ordinary constructor expression whose non-constructability is decided at runtime; template-continuation tokens delimit an empty `yield` operand before the continuation expression; AST variants and parser organization may change while these observable grammar and layering properties remain intact | PR #561 closes three audit residuals without adding representation guards; PR #643 closes the constructor and template-continuation boundaries |
| Frontend source and token ownership | well-formed source retains one owned UTF-8 rendering, while dynamic compilation of an ill-formed ECMAScript string adds only a sparse byte-offset map for its lone UTF-16 surrogates; checked UTF-8 cursor boundaries remain the diagnostic coordinate owner, and string, template, and RegExp tokens recover exact code units before immutable shared static-value interning; source scanning recognizes ECMAScript U+FEFF whitespace explicitly, and braced Unicode escapes are bounded by the decoded code-point value rather than a source digit-count surrogate; speculative replay and parser consumption reuse those payloads, `CompiledScript` discards the complete source, and each ordinary function record retains only its exact source slice for `Function.prototype.toString`; dynamic constructors separate their anonymous compilation wrapper from the specified named display source, including the display-only parameter-list line terminator; the `SourceRecord` ledger reserves count and payload before function insertion, replaces generated display source transactionally, and releases both through GC reconciliation; cursor, smart-pointer, source-mode, token-buffer, sparse-map, and interning representations may change as long as exact UTF-16 and function-source semantics remain observable without restoring redundant complete-source ownership; behavioral source tests protect that contract instead of a helper-call spelling guard | PR #562 removes the audit allocation copies; PR #631 extends exact ill-formed dynamic source; PR #632 adds exact ordinary-function source ownership; PR #642 separates generated compilation and display forms; PR #643 closes the U+FEFF and braced-escape residuals |
...
| Built-in argument failures | ordinary ECMAScript validation uses typed exception requests: Array constructor numeric lengths reject fractional, negative, non-finite, and above-`2^32-1` values with catchable `RangeError`, while Symbol description coercion reuses `ToString` and its catchable `TypeError`; `Error::runtime` remains reserved for engine invariants | PR #575 closes the independent residuals from the audit. Callable-valued prototype and HostFunction property gaps remain explicit input to the callable-as-object architecture task rather than being hidden behind a semantically wrong typed exception; error-site representation and helper layout are not guarded |
| Legacy `__proto__` semantics | ordinary reads, writes, compound assignments, deletes, and member calls use the shared descriptor, accessor, semantic-prototype, and cache owners; lazy `Object.prototype` materialization is prepared at the ordinary semantic boundary, so cache and bytecode paths do not recognize the property name independently | PR #529 installs the configurable Annex B accessor and PR #576 removes its residual optimizer and physical-path exceptions. Only the object-literal prototype initializer and the intrinsic definition retain the spelling; helper, cache, and bytecode representations remain replaceable |
...
| Source diagnostics | `CompiledScript` owns deterministic `SourceId`, an optional bounded source name, and parser-derived strictness and top-level-await execution bits; those execution bits retain neither source text nor AST. Tokens, lexer/parser errors, every recursive frontend AST node, and instruction-aligned bytecode metadata carry canonical `SourceSpan` ranges; ordinary functions retain only their exact source slice for observable `Function.prototype.toString`; identifier tokens retain escape provenance so shared keyword gates reject escaped ReservedWord spellings and contextual recognizers accept only unescaped names; the provenance representation may change while property `IdentifierName` grammar remains distinct from keyword grammar; structured runtime diagnostics expose the executing range | AS-04b2a merged in PR #419; AS-04b2b1 merged in PR #420; AS-04b2b2 merged in PR #421 without retaining complete source text or AST; AS-09l extends the guarded token metadata without adding another source range; PR #484 adds only strictness metadata; PR #557 centralizes escaped keyword and contextual-name decisions; PR #566 adds only the top-level-await execution bit; PR #632 adds exact ordinary-function source slices |
...
while `CompiledScript` retains neither the complete source text nor AST.
...
The AST and complete top-level source are still discarded before runtime, and
the architecture guard rejects a second source map or an unspanned frontend
root. Ordinary functions retain only their exact source slice so
`Function.prototype.toString` does not reconstruct source from syntax.
...
No runtime owner retains tokens, AST, or a redundant complete script source;
ordinary function records own only the exact source slice required by
...
| bindings and closures | globals, builtin globals, activation-owned upvalue frames, captured `with` object-environment chains, typed eval binding environments, `BindingCell(Rc<RefCell<Binding>>)` | binding values, captured cells, object environment values, and active eval-variable cells | logical Binding, ExecutionFrame, and binding-index CacheEntry counts |
| executable functions | sparse function, native-function, bound-function, and host-function arenas plus the native registry | typed AS-05b1b1 edges cover JavaScript/native/host properties and inheritance, upvalues, super/static/class/new-target state, native id payloads, and bound args/targets; immutable bytecode contains only VM-independent literals; opaque callback captures use AS-05a2d retained handles | logical JavaScriptFunction/NativeFunction/BoundFunction/HostCallback counts plus callable property, metadata-cache, binding, and source-record counts; AS-07 sweeps unmarked arena records |
| objects | sparse `ObjectHeap`, shapes, prototypes, properties, arrays, buffers, typed-array views, and an object-indexed typed host-payload registry | typed AS-05b1b2 edges cover named/dense/sparse properties, accessors, prototypes, boxed strings/Symbols, Proxy state, typed-array buffer-object links, and explicitly declared host-payload values; cached prototypes and shape/property-key metadata are direct anchors | logical Object/ObjectProperty/ByteBuffer/HostInstance/HostPayload counts plus exact host logical payload bytes, shape CacheEntry, and anchor Association counts; AS-07 recomputes live payload counters after sweep |
| collections | collection stores with retained kind, object slots, iterator snapshots, FinalizationRegistry cells, and WeakRef targets | typed object associations, strong Map/Set entries and iterator items, weak WeakSet keys, WeakMap ephemerons, strong finalization callbacks and held values, and weak finalization targets, unregister tokens, and WeakRef targets | logical Collection/CollectionEntry/CollectionIterator/IteratorItem and Association counts plus asynchronous edge snapshot |
...
| active execution | one `activation_frames` stack owns stable call function ids independently of continuation parking, call local bases, upvalues, dynamic object environments and shared eval-environment handles, `this`, `new.target`, super, eligible legacy-function argument snapshots and their lazily materialized per-call objects, an owned-block continuation program key, parked interpreter state, a resumable child completion, typed structured-control records, resumable destructuring tasks, and resumable sync/async `yield*` iterator state plus temporary-this/eval-boundary/standalone-bytecode variants; await suffixes move unchanged into reactions or async-generator associations, while both generator kinds retain the same detached activation representation | explicit call/block activations, active and suspended function program roots, original legacy-call argument values and materialized arguments objects, active eval-variable cells, parked bytecode/control/pattern/generator operands, live destructuring and delegated iterators, queued async-generator requests, and scoped traceable operand/call/iterator/descriptor/Proxy values | one logical ExecutionFrame per activation, lexical scope, object dynamic environment, and structured-control record; retained legacy-call argument values are charged as activation Bindings until unwind, each shared eval binding environment and its index are counted once through VM-owned weak identity regardless of the number of capturing functions or detached activations, alongside suspended-owner reconciliation, Promise/Association request counts, TransientRoot counts, and a fourteen-category root snapshot |
...
| embedder-visible state | output, host callbacks, typed host payloads, VM-owned dynamic module loader, `Vm`, public `Value`, retained registry | callback arguments are scoped roots; the loader is an embedder-owned source capability with no JavaScript `Value` edge; opaque captures and durable results use retained handles; typed payload values are edges from their ordinary wrappers rather than independent roots; raw Values remain compatibility-only and non-durable | logical HostCallback/HostInstance/HostPayload/RetainedHandle/OutputEntry counts and exact host logical payload bytes; retained handles also participate in root snapshots |
...
Typed host wrappers do not add another JavaScript object kind or semantic
dispatch path. Creation allocates `Object::ordinary`, then records a checked
object-to-payload association in a side registry. Property, descriptor,
prototype, Proxy, callability, and future subclass behavior therefore continue
through the existing semantic-object owners. An explicit clone creates a new
ordinary wrapper and increments `HostInstance`, while the shared Rust allocation
and its logical bytes remain one `HostPayload`. Declared VM-local values are
visited as `InternalSlot` edges from every live wrapper, not retained-handle or
runtime-anchor roots, so payload-to-JavaScript cycles remain visible to the
ordinary marker and collectible. Creation, clone, retained-root admission, and
payload-byte replacement are transactional; final wrapper collection removes
the payload exactly once, while consuming VM teardown relies on Rust ownership
to release the same allocation deterministically. The registry representation
may change while ordinary wrapper semantics, VM identity checks, explicit
...
anchors; registered reusable native-function ids; active async-function result
Promise ids; and result Promise ids plus handler/settled values in queued
Promise jobs. These are roots owned independently of heap traversal.
...
repeated calls and reconciles the resulting storage ledger. Eval-created cells
are registered in a traced, storage-counted environment captured by functions
created during that eval; the caller VariableEnvironment remains the canonical
cell owner, while the environment supplies dynamic lookup and configurable
deletion semantics without exposing caller frames to unrelated functions.
...
- `Vm::create_host_function` and its typed variant allocate ordinary
  VM-local `HostFunction` values without creating global bindings. Allocation
  and retained-root creation form one transaction: retained-limit rejection
  removes the new arena entry and releases its property storage. Once passed
  into JavaScript, ordinary property and callable edges keep the Rust capture
  reachable through the existing trace and GC owners; deleting the final edge
  permits collection and capture destruction;
...
7. Promise state/reactions, collection entries, iterator state, finalization
   registry cells, and WeakRef targets with weak keys and targets classified
   separately: executable in AS-05b1b3;
...
10. persistent module scopes, namespaces, source objects, import metadata,
    evaluation promises/results/errors, and detached top-level continuations:
    executable through AS-09ah and AS-09an.
...
| AS-05b1b3 | Promise, collection, iterator, weak-key, ephemeron, finalization-registry, and WeakRef edges | typed side-store associations, fifteen strength-classified categories, and bounded Context/Vm diagnostics; the weak-lifecycle extension has direct behavioral snapshot coverage |
...
| AS-09an | asynchronous module graph evaluation ownership | Draft PR #636 replaces per-module synchronous Promise draining with one VM-owned graph scheduler. Every persistent module owns one evaluation capability, result/error, pending-dependency count, cycle root, and at most one detached top-level continuation; dependency, alias, dynamic-import, and top-level-await reactions all enter the shared FIFO Promise job queue. Sibling modules start before an importer waits, asynchronous transitive dependencies discovered through a deferred edge join the same importer settlement boundary, strongly connected components expose one external wait target, canonical module records reuse terminal live-binding cells and settlement, and dependency rejection preserves the original JavaScript error. An explicit top-level module execution boundary supplies module `this` semantics during initial and resumed bytecode; source names remain diagnostic identity and never select the parse or execution goal. Detached execution is included in direct roots, storage reconciliation, and cancellation. Realm-sensitive module-source constructors are reached through one explicit host capability that validates a VM-owned realm global; child realms do not implicitly inherit unrelated host extensions. `GetModuleSource` has one linking boundary, and an unavailable Source Text Module representation remains a `SyntaxError` through dynamic-import rejection. The exact module/source/await-using profile advances from 764/794 to 794/794 variants. |
| AS-09ao | ECMA-402 locale-list and locale-aware collection dispatch ownership | Draft PR #640 moves `CanonicalizeLocaleList` out of an individual formatter into one shared Intl owner used by every service constructor. NumberFormat and DateTimeFormat no longer maintain weaker parallel tag parsers, so primitive boxing, invalid-entry rejection, alias canonicalization, duplicate removal, canonical casing, and Unicode extensions enter formatter negotiation through the same boundary. Array and TypedArray locale-string methods preserve their distinct length owners while sharing the requirement to invoke each element with exactly the original `locales` and `options` values. The Test262 realm handle exposes its target-realm `evalScript` capability alongside `global`, without introducing process-global realm state. The complete focused `intl402` profile advances from 6,662/6,682 to 6,682/6,682 variants. |
| AS-09ap | binary-view exotic lifecycle and construction-order ownership | Draft PR #641 routes TypedArray integrity operations through the integer-indexed semantic owner instead of ordinary-object fast paths. Fixed views over ordinary resizable buffers and all length-tracking views reject `[[PreventExtensions]]`, while fixed views over grow-only shared buffers retain stable indexed keys. ArrayBuffer, SharedArrayBuffer, and TypedArray construction resolve an observable custom `new.target.prototype` before backing-store allocation or later argument validation where required. TypedArray buffer construction and `set` snapshot their structural inputs before observing detach-sensitive backing-store state, preserving error precedence and later source side effects. The six exact focused profiles close all 16 previously failing variants across variable-length integrity, allocation ordering, constructor sequencing, and detached-source copying. |
...
| direct eval variable environments | one typed `EvalVariable` binding-layout operand distinguishes eval vars from lexical globals; every sloppy direct-eval-capable activation creates one live shared binding environment before nested function capture, call activations distinguish captured from active dynamic environments, and resolution orders eval-local scopes, active dynamic environments, caller function locals, then captured dynamic environments; later eval declarations append storage-counted deletable entries rooted in the caller VariableEnvironment, so functions created before or during eval observe the same cells without duplicating them as captured declarative bindings; simple catch parameters retain their intervening lexical priority while the redeclared var remains in the Function VariableEnvironment; lexical arrows preserve direct-eval `new.target` permission, and pre-setup global loads bypass caller-local dynamic environments | creating the environment only when eval runs, treating eval vars as globals, selecting the top lexical scope as VariableEnvironment, consulting caller activations for an unrelated function, resolving a pre-setup global load through a caller-local scope, losing shared eval environments or cells from tracing/accounting, counting the same mutable environment once per capture, duplicating eval-owned cells in a captured declarative scope, or retaining deleted eval bindings as declarative bindings |
| function name inference | one `compile_expr_with_inferred_name` compiler owner and one runtime `set_function_name` owner; static names travel as bytecode metadata, computed keys use the resolved property key, Annex B `for-in` initializers enter the same assignment-inference path before the RHS, and the parser's typed object-property kind keeps the object-literal `__proto__` setter explicitly outside inference while a distinct bytecode operand carries that setter fact to runtime without a source-name comparison | another naming owner, parser mutation of anonymous definitions, member/parenthesized assignment inference, Symbol display text used instead of bracketed descriptions, naming the `__proto__` setter value, a compiler or runtime source-name heuristic, treating a method/accessor named `__proto__` as a prototype setter, or an accessor prefix assembled outside the shared runtime owner |
| script and global declaration evaluation | host script evaluation owns an explicit activation boundary and performs one complete GlobalDeclarationInstantiation preflight before mutating any lexical binding or global object property; lexical conflict checks read virtual global descriptors without materializing the lazy global object, while object-record declarations create it when required | executing host scripts in an ambient caller activation, partially applying declarations before a later conflict, recognizing declaration conflicts after mutation, or allocating `globalThis` merely to validate an unrelated lexical declaration |
| realm-owned intrinsic state | mutable intrinsic observations belong to the active `RealmState`; Annex B legacy `RegExp` statics retain VM-owned exact UTF-16 input and match subjects plus capture spans, expose both strings as direct roots, and reconcile every retained association through the VM storage ledger; unsuccessful matches preserve the previous successful state and child realms remain independent | process-global builtin state, sharing legacy match observations between realms, retaining unrooted strings or unaccounted capture vectors, deriving contexts from UTF-8 offsets, clearing state after a failed match, or returning placeholder empty strings after a successful match |
| function realm and source ownership | each function activates its own static binding/cache layout; body execution and thrown values remain in the callee realm, while derived-constructor return normalization runs in the caller realm; ordinary function records retain their exact source slice and no complete script source; functions created by source-text modules retain their defining Script-or-Module name and a traced edge to the module-owned `import.meta` object, including declarations created during module instantiation | using the caller layout for a dynamically compiled function, normalizing a derived constructor in the callee realm, reconstructing function source, retaining the complete script source, or resolving `import.meta` and dynamic-import referrers from the caller module |
| callable prototype ownership | ordinary objects and every callable store retain semantic `Value` prototype edges; bound functions copy the target's current `[[Prototype]]`, and every property/prototype traversal re-enters semantic dispatch | reducing a prototype edge to `ObjectId`, forcing bound functions to `%Function.prototype%`, bypassing callable/null prototype values, or continuing through a physical-only prototype walker |
| indexed array write dispatch | dense and sparse index fast paths remain valid only while every prototype is an ordinary inert boundary; a Proxy, callable semantic object, or TypedArray prototype routes the write through the shared receiver-aware `[[Set]]` owner | writing an indexed element directly after crossing a Proxy, callable, or TypedArray boundary; suppressing an inherited setter or Proxy `set` trap; or assigning before the semantic receiver decision |
...
| built-in observation phases | URI decoders consume exact UTF-16 input and preserve every unescaped code unit, while percent escapes alone enter UTF-8 decoding; Date setters coerce every supplied argument left-to-right before date validity or numeric-result computation, and flexible space-separated legacy date forms remain separate from strict `T`-form parsing | round-tripping URI input through a lossy UTF-8 string, rejecting an unescaped lone surrogate, short-circuiting later Date argument conversion after `NaN`, or weakening strict ISO `T` field widths to accept a legacy space-form extension |
| iterator-helper receiver ownership | `%IteratorHelperPrototype%` owns the shared `next` and `return` methods, while each helper object owns only a traced state token; either realm's prototype methods recover the helper state from the receiver before entering the shared stepping and closing owners | per-instance public helper methods, a method closure that fixes one helper id, accepting a non-helper receiver, or preventing a helper prototype method from one realm from operating on a helper created in another realm |
| collection construction ownership | Map, Set, WeakMap, and WeakSet resolve the semantic `new.target.prototype` before allocating and seeding the instance; iterable seeding reads the dynamic adder from that final prototype chain, and every post-step entry/adder failure enters the shared iterator-close owner while retaining the original throw | applying a subclass prototype only after seeding, reading the base intrinsic adder for a subclass, requiring an ordinary-object-only prototype, omitting iterator close after an entry or adder error, or replacing the original throw with a non-resource close failure |
| Unicode case-mapping ownership | ordinary and locale-aware String case conversion share the ICU Unicode data owner over exact UTF-16 input; valid scalar runs retain contextual and locale-sensitive mappings, while unpaired surrogate code units remain unchanged at their original positions | using the Rust standard-library Unicode snapshot for ordinary conversion, replacing unpaired surrogates with U+FFFD, or letting ordinary and locale-aware methods drift onto separate mapping tables |
| binary16 numeric ownership | one checked ties-to-even IEEE-754 binary16 codec owns conversion for `Math.f16round`, DataView `getFloat16`/`setFloat16`, and `Float16Array`; the typed-array element kind participates in the same constructor, storage, species, cross-realm, sorting, and view lifecycle as the other numeric kinds | separate Float16 rounding implementations, a name-only global, a Float16 view that bypasses the shared typed-array record, or exposing Float16 to the common harness before all generic typed-array methods accept the kind |
| semantic class-prototype ownership | class heritage validates the superclass `prototype` through the shared semantic-object boundary, roots both heritage values across allocation, and installs the exact object-like value as the class prototype's parent; ordinary `instanceof` compares semantic prototype values across ordinary, bytecode-function, native-function, host-function, and Proxy storage | narrowing a valid prototype to `ObjectId`, rejecting a callable host exotic, retaining a heritage value outside a VM root during allocation, or teaching class creation and `instanceof` different definitions of object identity |
| execution-limit ownership | parser expression nesting and active ECMAScript call depth are independent limits; exhausting the JS call stack throws a catchable `RangeError`, while runtime steps, host quotas, allocation limits, and integer overflow remain non-catchable embedder failures | using a syntax-depth field as the runtime call-stack boundary, exposing a host resource failure to `catch`, allowing call depth to reach the Rust stack limit, or relying on process-global stack settings instead of a per-VM limit |
| frontend lowering facts | binding layout owns the single `for` lexical-scope predicate; parser nodes retain strictness needed by lowering; switch case tests and bodies share one initialized lexical scope and one resumable selection record; sloppy `let` first distinguishes StatementListItem from Statement grammar and then uses declaration lookahead independent of line terminators; initialized `var` declarations use the assignment-reference lowering so `with` and direct-eval-sensitive bindings resolve before right-hand-side effects, then discard the assignment value at the declaration boundary; Annex B block-function suppression derives `parameterNames` from both formal parameters and the materialized implicit `arguments` binding | a compiler-side copy of a layout/parser fact, case selection before switch lexical initialization, loss of selection state across suspension, line-terminator-based `let` declaration disambiguation, resolving a variable binding after its initializer, or allowing an Annex B update to replace a parameter or implicit `arguments` binding |
| optional-chain completion ownership | one recursive lowering owns nullish exits across member access, calls, receiver preservation, and deletion; ordinary value chains substitute `undefined`, parenthesized calls retain the required receiver/callee stack shape, and delete chains perform the final semantic `[[Delete]]` while substituting `true` without evaluating skipped keys or arguments; the parser rejects optional chains directly following an argument-less `new` constructor target and rejects private-member deletion | compiling delete as a read followed by `DeleteValue`, substituting `undefined` for a short-circuited delete, evaluating a computed key after a nullish exit, losing a call receiver, accepting `new value?.member` or `delete value?.#private`, or adding a second chain walker |
...
| module import live bindings | the frontend exposes typed module-only import-binding facts to layout and hoisting without forging ordinary declarations or recognizing source names; named, namespace, and source-phase import forms remain typed before compiled metadata is built, so an ordinary string import name such as `"*"` cannot collide with a syntactic namespace marker; those bindings are instantiated uninitialized, omitted from executable declaration bytecode, and linked to terminal local or namespace cells before evaluation; export-of-import metadata is normalized to indirect or namespace exports, and runtime aliases preserve both live updates and TDZ through cycles; the concrete frontend node, cell, and alias representations may be replaced | filtering ordinary declarations by import-name strings, encoding namespace syntax as an ordinary import-name string, executing an import binding as a declaration, linking an import to another import alias, copying an exported value instead of retaining its live binding, or bypassing TDZ for a cyclic import read |
| asynchronous module evaluation | each persistent module has one evaluation capability and at most one detached continuation; eager dependencies and asynchronous transitive dependencies discovered through deferred edges share the importer settlement boundary, dependency and await settlement resumes through the shared FIFO Promise job owner, canonical records share terminal binding cells and settlement, external importers observe a cycle root, initial and resumed top-level bytecode enter one explicit module execution boundary, realm-sensitive module-source intrinsics require an explicit capability plus a validated VM-owned realm global, unavailable Source Text Module records preserve `SyntaxError` through static linking and dynamic-import rejection, and every detached scope/frame participates in roots, storage reconciliation, and cancellation; scheduler records and graph algorithms may change | draining jobs independently inside a module body, evaluating an importer before a pending dependency or cycle root settles, copying canonical exports, recreating dependency errors, converting a missing source representation into a generic runtime `TypeError`, using a source name as an execution-goal discriminator, implicitly inheriting host extensions into child realms, resuming outside the Promise queue, or parking module execution outside root/accounting/cancellation ownership |
...
| asynchronous edge boundary | typed categories and strength visitors represent Promise state/reactions, object associations, Map/Set entries, iterator items, WeakSet keys, WeakMap ephemerons, FinalizationRegistry callback/cell edges, and WeakRef targets; `tests/async_edge_snapshot_smoke.rs` verifies strong, weak, and ephemeron behavior independently of source ownership | removing a typed category or visitor protocol, treating weak keys, targets, unregister tokens, or ephemerons as ordinary strong entries, exposing raw association ids publicly, or losing behavioral edge coverage; source files, helper ownership, and match layout remain replaceable |
| async function completion | one settle-or-suspend owner handles both initial execution and every resumed await; the active result Promise is a direct root while that owner runs; settlement resolves or rejects the result Promise, while suspension detaches the current activation before registering its await reaction; completion and continuation representations may change | separate initial/resume completion dispatch, reaction registration before detachment, an unrooted active result Promise, or a resumed completion that bypasses the shared result-Promise owner |
| delegated yield transport | synchronous delegation carries the iterator-result object, while asynchronous delegation carries a typed value plus its await-before-yield decision; internal control data is never encoded in JavaScript-visible object properties, and the Rust representation may change | one untagged payload with consumer-dependent meanings, property reads used to recover internal protocol flags, or suspension transport that omits the carried value from root enumeration |
| execution suspension layering | the specification completion record carries one typed suspension signal, and the bytecode boundary reports only completed or suspended execution; generic propagation treats suspension as opaque, async and generator owners inspect only the kinds they settle, and the suspension owner enumerates every carried root; the concrete enum and boundary representations may change | generic completion consumers enumerating await or yield kinds, parallel bytecode outcomes for each kind, an owner accepting a suspension kind it cannot settle, or rooting suspension payloads outside the typed owner |
