...
| AS-09u | Promise settlement combinators and AggregateError ownership | Draft PR #483 represents `all`, `allSettled`, `any`, and `race` with one combinator kind and one traced element-function state owner. The methods reuse generic capability, observable resolve, iterator-close, reaction, and array owners; generic Promise resolve/reject and AggregateError errors/cause/prototype semantics share the same boundary. The six exact profiles advance from 102/724 to 712/724 variants and from 51/362 to 356/362 files; residuals belong to BigInt syntax, `Promise.prototype.finally`, or the cross-realm `$262` harness. |
