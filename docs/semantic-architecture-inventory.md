...
| ArrayBuffer | `byte_buffer` on `Object`; AS-09y shares one `Rc<RefCell<ByteBufferState>>` containing optional live bytes and a resizable maximum across the buffer and all views | branded ArrayBuffer accessors/methods plus ordinary properties; resize/detach update the ObjectHeap byte payload total |
...
| AS-09ab | update-expression numeric coercion ownership | PR #501 merged as `af014a28` and routes binding, static, computed, array-index, private, super, cached, linear, and direct `++`/`--` execution through one `bytecode_update_values` owner backed by shared `ToNumber`. The owner returns both the converted previous number and updated number, so postfix results do not repeat observable conversion. Computed references reject a nullish base after evaluating the property expression but before `ToPropertyKey`. Exact-tree run `29188157482` certified merge tree `0ff87c5c`, and post-merge run `29188096231` attempt 2 published canonical report `20260712T095437Z` in `8f1fb7fa`; the baseline gained 134 variants / 67 files with no removed pass. |
| AS-09ac | Number prototype internal-slot and formatting order | Draft PR #504 represents `%Number.prototype%` with the existing boxed-number payload and `+0` data instead of an ordinary object, so receiver validation, primitive conversion, object tagging, and inherited methods share the same physical owner as constructed Number objects. `toExponential` and `toPrecision` convert provided digit arguments before returning non-finite text while retaining non-finite precedence over range validation. The exact Number profile advances from 566/680 to 670/680 variants and from 283/340 to 335/340 files through 104 additions with no removed pass; residuals require BigInt syntax, realm support, or the separately owned Array-to-primitive path. |
...
| strict property writes | one parser-owned strict assignment-target validator and one strictness bit on every ordinary bytecode property reference; optimized chains accept only explicit sloppy instructions, while every generic strict write delegates rejection to the shared semantic `Set` owner | a property assignment, update, compound, logical, or destructuring path that drops strictness, bypasses `Set`, accepts restricted strict identifiers, or lets postfix update cross a line terminator |
| update numeric coercion | one bytecode runtime owner performs shared `ToNumber` and returns the numeric previous/updated pair; every binding, property, array, private, super, cached, linear, and direct update path delegates to it | another update conversion owner, an already-number-only update path, repeated observable conversion, a postfix result that preserves the unconverted operand, or computed-key conversion before nullish-base rejection |
