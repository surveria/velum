...
| AS-09aj | VM-local realm state ownership | Draft PR #522 begins the VM/realm split by moving global and builtin binding scopes, the global object, the native intrinsic registry, and generator/async/promise intrinsic prototypes behind one explicit `RealmState`. Heap arenas, atoms, symbols, functions, jobs, limits, accounting, and VM identity remain on `Context` as VM-shared owners. All access sites are explicit `self.realm` paths, while direct-root and storage-accounting guards continue to require the same live sources; this first checkpoint intentionally preserves single-realm behavior before adding realm ids, switching, and public creation/evaluation APIs. |
