...
| AS-09r | `Promise.all` constructor and abrupt-completion semantics | draft PR #478 routes generic capabilities through shared construct/call owners, preserves derived Promise prototypes, assimilates thenables through the Promise job owner, and performs specification-ordered iterator closing and idempotent element resolution. The exact standard `Promise.all` profile advances from 114/196 to 194/196 variants and from 57/98 to 97/98 files; the sole residual file requires unsupported BigInt syntax. |
