...
| AS-09al | frontend binding and lowering fact ownership | Draft PR #535 replaces the compiler's divergent `for` lexical-scope predicate with the binding-layout owner, stamps parser strictness onto ordinary `for-in`/`for-of` assignment targets and delete references, and makes sloppy `let` disambiguation depend on declaration lookahead instead of line terminators. Switch selection now creates and initializes its lexical environment before case tests, with the selection phase, discriminant, and suspended test state retained by the existing continuation-owned control record. Focused engine coverage pins all five boundaries, including an awaited case test inside a lexical switch scope. |
