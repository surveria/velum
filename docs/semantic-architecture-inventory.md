...
| AS-09w | Promise keyed combinator dictionary ownership | Draft PR #486 adds `allKeyed` and `allSettledKeyed` to the shared combinator kind and traced element-state owner. Shared semantic own-key and descriptor operations select enumerable string/Symbol keys in observable order; one keys/value state builds a null-prototype result with ordinary descriptors, and settled entries reuse the standard settlement-record owner. Focused coverage advances from 4/178 to 172/178 variants and from 2/89 to 86/89 files; four residual variants require BigInt syntax and two expose destructive ordering in the upstream descriptor helper. |
