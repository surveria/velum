...
| AS-09z | Test262 assertion harness contract | Draft PR #494 keeps the runner-owned syntax-compatible `assert.js` replacement aligned with the upstream mandatory globals rather than adding engine/compiler special cases. One compatible source owns `isNegativeZero`, `isPrimitive`, formatting helpers, global and assertion array comparison, and SameValue behavior; the exact TypedArray profile advances from 702/2,876 to 1,368/2,876 variants and from 351/1,446 to 688/1,446 files. |
