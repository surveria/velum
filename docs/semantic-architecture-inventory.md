...
| AS-09ah | module source goal and graph ownership | Draft PR #517 adds a strict module parse goal, public compiled import/export metadata, an embedder-controlled canonical source loader, VM-local graph instantiation, persistent module scopes, live import aliases, cycle-aware evaluation, and stable sealed namespace objects. Module records are now real checked `VmStorageKind::Module` owners instead of the prior explicit-zero placeholder; their scopes and namespace values participate in the existing direct-root boundary. Top-level await and Test262 runner routing remain in progress. |
