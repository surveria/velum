...
| asynchronous module evaluation | each persistent module has one evaluation capability and at most one detached continuation; dependency and await settlement resumes through the shared FIFO Promise job owner, canonical records share terminal binding cells and settlement, external importers observe a cycle root, initial and resumed top-level bytecode enter one explicit module execution boundary, and every detached scope/frame participates in roots, storage reconciliation, and cancellation; scheduler records and graph algorithms may change | draining jobs independently inside a module body, evaluating an importer before a pending dependency or cycle root settles, copying canonical exports, recreating dependency errors, using a source name as an execution-goal discriminator, resuming outside the Promise queue, or parking module execution outside root/accounting/cancellation ownership |
