...
| `[[SetPrototypeOf]]` | `semantic_try_set_prototype` | ObjectHeap, Function, and HostFunction stores retain semantic `Value` prototypes with extensibility and cycle checks; ordinary traversal re-enters semantic dispatch at every callable, Proxy, or object hop | AS-02b2 complete in PR #403; callable-valued ordinary prototype storage completed by PR #632 |
...
| direct eval variable environments | one typed `EvalVariable` binding-layout operand distinguishes eval vars from lexical globals; sloppy functions record their own direct-eval capability, call activations distinguish captured from active dynamic environments, eval-created closures retain traced captured lexical cells plus one storage-counted deletable binding environment rooted in the caller VariableEnvironment, lexical arrows preserve the direct-eval `new.target` permission, and pre-setup global loads bypass caller-local dynamic environments while ordinary execution retains Annex B catch shadowing | treating eval vars as globals, selecting the top lexical scope as VariableEnvironment, consulting caller activations for an unrelated function, resolving a pre-setup global load through a caller-local scope, losing eval cells from tracing/accounting, or retaining deleted eval bindings as declarative bindings |
