...
| active execution | one `activation_frames` stack owns call local bases, upvalues, `this`, `new.target`, super, a function-id or owned-block continuation program key, parked interpreter state, a resumable child completion, typed structured-control records, resumable destructuring tasks, and resumable `yield*` iterator state plus temporary-this/eval-boundary/standalone-bytecode variants; await suffixes move unchanged into reactions, while synchronous generators retain the same suffix in a generator association | explicit call/block activations, active and suspended function program roots, parked bytecode/control/pattern/generator operands, live destructuring and delegated iterators, and scoped traceable operand/call/iterator/descriptor/Proxy values | one logical ExecutionFrame per activation, lexical scope, and structured-control record plus suspended-owner reconciliation, TransientRoot counts, and a fourteen-category root snapshot |
...
| destructuring assignment | one typed `AssignmentPattern` frontend path and one resumable `BytecodePattern` runtime walker shared with binding patterns; assignment references resolve before iterator/property reads and remain rooted through defaults and `PutValue` | a second bytecode/runtime pattern tree, assignment-only walker, target evaluation after source reads, loss of iterator close, or assignment mode that discards the RHS result |
