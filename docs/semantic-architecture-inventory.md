...
| AS-09l | asynchronous `for await...of` iteration | draft PR #466 adds an explicit async mode to the shared `ForOf` AST/bytecode/control path, awaits async iterator results and sync-fallback values through the Promise job owner, performs resumable destructuring assignment, and awaits `AsyncIteratorClose`; the focused profile advances from 178/2,433 to 2,427/2,433 variants and from 92/1,235 to 1,230/1,235 files. Four residual variants require block-scoped function-declaration instantiation and two require the `$262.createRealm` harness. |
