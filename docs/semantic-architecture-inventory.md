...
| direct eval variable environments | one typed `EvalVariable` binding-layout operand distinguishes eval vars from lexical globals; sloppy functions record their own direct-eval capability, call activations retain a stable function id while continuations are parked, and eval-created closures capture one traced and storage-counted deletable binding environment rooted in the caller VariableEnvironment | treating eval vars as globals, selecting the top lexical scope as VariableEnvironment, consulting caller activations for an unrelated function, allowing a pre-setup fast path to resolve context-dependent global operands, losing eval cells from tracing/accounting, or retaining deleted eval bindings as declarative bindings |
