...
| asynchronous edge boundary | typed categories and strength visitors represent Promise state/reactions, object associations, Map/Set entries, iterator items, WeakSet keys, and WeakMap ephemerons; `tests/async_edge_snapshot_smoke.rs` verifies collection behavior independently of its source owner | removing a typed category or visitor protocol, treating weak keys/ephemerons as ordinary strong entries, exposing raw association ids publicly, or losing behavioral edge coverage; source files, helper ownership, and match layout remain replaceable |
