...
| AS-09ab | update-expression numeric coercion ownership | Draft PR #501 routes binding, static, computed, array-index, private, super, cached, linear, and direct `++`/`--` execution through one `bytecode_update_values` owner backed by shared `ToNumber`. The owner returns both the converted previous number and updated number, so postfix results do not repeat observable conversion. Computed references reject a nullish base after evaluating the property expression but before `ToPropertyKey`. The exact prefix/postfix profile advances from 118/246 to 230/246 variants and from 74/142 to 130/142 files through 112 additions with no removed pass; residuals require BigInt syntax, eval, or a separate global object-environment deletion fix. |
