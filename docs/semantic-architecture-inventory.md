...
| Frontend lexical goals | one checkpointed token stream scans on demand; parser expression entry selects `InputElementRegExp`, division and division-assignment grammar select `InputElementDiv`, and speculative lookahead may be invalidated before its token is accepted | draft PR #545 replaces eager previous-token slash authority while retaining one buffered token and source-span owner |
