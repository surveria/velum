...
| AS-09ae | arbitrary-precision BigInt primitive and numeric ownership | Draft PR #509 adds one immutable ownerless `JsBigInt` payload, scans literals without binary64, and routes `ToNumeric`, arithmetic, bitwise, comparison, update, boxing, constructor, and prototype behavior through shared owners. `RuntimeLimits::max_bigint_bits` bounds materialized results, while zero and unit identities avoid width-dependent allocation. The exact built-ins/BigInt profile reaches 152/154 variants with only the separately owned `$262.createRealm` pair remaining; the broad lowercase profile advances from 80/1,050 to 476/1,050, with all 292 language-expression and 118 literal variants passing. BigInt64/BigUint64 typed arrays, DataView consumers, SharedArrayBuffer, and Atomics remain a subsequent payload tranche. |
