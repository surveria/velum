...
| Dense arrays | Partial foundation. Arrays now use a dedicated `ArrayStorage` with packed and holey dense elements plus sparse index keys. Guarded packed fast paths exist for scan/copy-style reads, concat, join, slice, default-attribute reverse, and default-attribute shift/unshift through deque-backed front operations. Slice-style fast paths still require contiguous packed storage and fall back when front operations wrap the deque. | Reduce the remaining array-method latency debt with prototype-index guards, descriptor-aware mutation paths, direct bytecode/native loops, and explicit materialization points. Preserve fallback for holes, sparse indices, descriptors, and prototype index properties. |
...
Holey arrays need explicit missing-element semantics. Sparse arrays,
descriptor-modified arrays, arrays affected by prototype index properties, and
non-contiguous packed storage for slice-style operations must fall back to the
generic object/property path or materialize explicitly.
