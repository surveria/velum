...
| Slot-based bindings | Partial. `BindingScope` stores cells in a `Vec` behind an atom-to-slot map, and the AST now carries static names through a typed `StaticName` operand instead of raw `String` fields. | Add compile-time scope layout with `LocalSlot`, `GlobalSlot`, and `UpvalueSlot`, then make ordinary local access index-based. |
| Atom interner | Partial. VM-local atoms cover lexical/global binding names and the atom table now uses a sorted vector index instead of a tree map; static AST names are typed so later property and binding operands can attach atom/slot metadata without changing parser surfaces again. | Extend atoms to property names, built-in names, function names, string literal keys, shapes, prototype lookup, and diagnostics so hot paths do not fall back to `String`. |
