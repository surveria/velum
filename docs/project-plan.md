...
| Shapes / hidden classes | Partial foundation. Ordinary object and function properties are atom-keyed, slot-backed, vector-indexed, ordinary object named-property lookups resolve slots through VM-owned `ShapeId` metadata, and a VM-local prototype lookup version now guards structural chain changes. | Add descriptor-aware transitions and cacheable lookup APIs for objects and prototypes. |
