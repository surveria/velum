...
| Atom interner | Partial. VM-local atoms cover lexical/global binding names and the atom table now uses a sorted vector index instead of a tree map. | Extend atoms to property names, built-in names, function names, string literal keys, shapes, prototype lookup, and diagnostics so hot paths do not fall back to `String`. |
