...
| [ ] | Backlog | Compiler-assigned slots and upvalues | Runtime architecture / performance | Replace repeated runtime name lookup for locals, globals, and captured variables with checked slot indices. | The compiled `BindingLayout` now exists, seeds matching runtime binding-cache occurrences after a safe runtime location is known, populates function parameter frames at compiled local slots, best-effort places lexical declaration paths at compiled local slots, best-effort hoists function-local `var` declarations into compiled local slots, and can best-effort place user top-level declarations into compiled `GlobalSlot` offsets because user globals and builtin globals are separate scopes. The next steps are closure/upvalue frame cells and direct slot operands for reads, writes, compound assignments, updates, and constructor lookups. Keep best-effort fallback for dynamic or mismatched runtime scopes, and keep exact global cache work guarded until Annex B and builtin-shadowing validation are explicit. |
...
now separate from lazy builtin globals, so top-level declarations can also
best-effort populate compiled `GlobalSlot` offsets without late builtin
materialization changing the user global frame. That is still not the final
design. The next slot work should add closure/upvalue frame cells and direct
slot operands for reads, writes, compound assignments, updates, and constructor
