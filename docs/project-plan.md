...
| Slot-based bindings | Partial. `BindingScope` stores cells in a `Vec` behind an atom-to-slot map, the AST separates parser-interned name text from per-occurrence `StaticBinding` operands, compiled scripts build a scope-aware binding layout with `GlobalSlot`, `LocalSlot`, and `UpvalueSlot` metadata, compiled function parameter scopes can be populated at compiled `LocalSlot` offsets, compiled lexical declarations, lexical `for...in` bindings, catch parameter scopes, and function-local hoisted `var` declarations can best-effort populate compiled `LocalSlot` offsets, user globals are separated from lazy builtin globals, top-level declarations can best-effort populate compiled `GlobalSlot` offsets, first global cache misses can build guarded locations directly from `GlobalSlot`, runtime local frames can carry a compiled `ScopeId` after slot-compatible insertion, first local cache misses can build guarded locations directly from `ScopeId + LocalSlot`, function objects can carry compact `UpvalueSlot -> BindingCell` frames, first upvalue cache misses can read directly from the active upvalue frame, compiled function AST nodes carry `StaticFunctionId` metadata, compiled layouts preserve function parent metadata, transitive upvalues are lifted through intermediate closures, simple and nested compiled closures with complete lifted upvalue frames can skip legacy captured scope snapshots, exact layout-backed local/upvalue cache entries no longer carry `AtomId`, active declaration cache seeding can use compiled slots before falling back to runtime location search, compound/update writes reuse the already resolved binding cell, and existing assignment, compound assignment, delete, and constructor lookup paths try the static binding cell before falling back to lazy builtin materialization. Layout-backed local/upvalue cache hits can skip ordinary shadowing scans. Global and builtin cache hits remain guarded because Annex B behavior can still expose runtime shadowing cases. | Add true bytecode-ready direct binding operands for the remaining AST-eval read/write paths and keep exact global read/write caches separate from Annex B validation work. Keep compact closure frames observable through resource counters while direct bytecode operands are introduced. |
...
| Shapes / hidden classes | Partial foundation. Ordinary object and function properties are atom-keyed, slot-backed, vector-indexed, ordinary object named-property lookups resolve slots through VM-owned `ShapeId` metadata, shapes include data-property descriptor attributes, cacheable named-property lookup snapshots exist for object/prototype reads and `in`, compiled static object-property reads, side-effect-free computed literal property reads, static own-property writes, and `in` presence checks can reuse guarded per-occurrence snapshots, and a VM-local prototype lookup version guards structural chain changes. | Add compiler-assigned property operands for the remaining dynamic property forms and extend guarded site caches beyond simple own-slot writes. |
| Inline caches | Partial. Compiled static member reads, side-effect-free computed literal member reads, method-call references, static own-property writes, and `in` presence checks now have per-occurrence cache slots instead of sharing lookup snapshots by static property name. Cached snapshots are still monomorphic and guarded by receiver identity, receiver shape, and prototype lookup version; writes only update existing own named data slots and fall back for missing, prototype, array-specific, `__proto__`, function/native, or stale paths. | Extend per-occurrence cache slots to broader static/computed calls, function/native dispatch, non-literal computed property keys, descriptor-aware writes, and bytecode property operands. Keep shape, receiver, descriptor, and prototype guards explicit with generic fallback for stale or uncacheable paths. |
| Dense arrays | Partial foundation. Arrays now use a dedicated `ArrayStorage` with packed and holey dense elements plus sparse index keys. Guarded packed fast paths exist for scan/copy-style reads, concat, join, slice, default-attribute reverse, and default-attribute shift/unshift through deque-backed front operations. Guarded holey dense fast paths now exist for `includes`, `indexOf`, `lastIndexOf`, `join`, `slice`, and `concat` when the prototype chain has no indexed own properties in the relevant range. Slice-style packed fast paths still require contiguous packed storage and fall back when front operations wrap the deque. | Reduce the remaining array-method latency debt with broader prototype-index guards, descriptor-aware mutation paths, direct bytecode/native loops, compact holey/sparse materialization, and explicit materialization points. Preserve fallback for holes, sparse indices, descriptors, and prototype index properties. |
| Built-in intrinsics | Partial. Constructors and prototypes are lazily materialized per VM. Function and native-function `length`, `name`, and `prototype` paths now use typed property-kind dispatch before descriptor handling, and immutable default `length`, `name`, and `prototype` descriptors are stored on `FunctionProperties` instead of rebuilt from function name/arity/prototype metadata on every descriptor or value read. | Separate the remaining immutable intrinsic metadata from mutable JS objects, pre-resolve atoms/shapes for built-ins, reuse descriptor object layouts, and add direct native-call paths for common built-ins. |
...
| Memory layout | Partial indexed handles for objects, functions, native functions, and host functions. A VM-owned `StringHeap` now provides `StringId` and heap-backed `JsString` handles for selected runtime-generated strings, including parser string/template literals, host-returned strings, `typeof`, concatenation results, `String(...)`, native function names, JSON strings, `Object.keys` results, and `for...in` keys. `Value` still supports owned strings for context-free diagnostics, string index-property results, and remaining externalized values. | Extend `StringId` into object/string wrapper paths, diagnostics where context is available, compact handles, Vec-backed heaps, free lists, boxed immutable constants, and captured-variable cells only where closures need them. |
...
| [ ] | Backlog | Compiler-assigned slots and upvalues | Runtime architecture / performance | Replace repeated runtime name lookup for locals, globals, and captured variables with checked slot indices. | The compiled `BindingLayout` now exists, seeds matching runtime binding-cache occurrences after a safe runtime location is known, populates function parameter frames at compiled local slots, best-effort places lexical declaration paths at compiled local slots, best-effort hoists function-local `var` declarations into compiled local slots, can best-effort place user top-level declarations into compiled `GlobalSlot` offsets, uses guarded `GlobalSlot` locations for first global cache misses, maps compiled local `ScopeId` metadata onto compatible runtime frames, materializes compact upvalue frames for escaping functions, lifts transitive upvalues through intermediate closures, stores atom-free exact cache locations for proven compiled local/upvalue hits, reuses already resolved binding cells for compound/update writes, avoids builtin materialization fallback when an existing static binding cell is available, and skips legacy capture snapshots for simple and nested compiled closures whose lifted upvalue cells are complete. The next steps are true direct slot operands for the remaining read/write/eval paths, bytecode-ready binding operands, and explicit exact-global cache validation. Keep best-effort fallback for dynamic or mismatched runtime scopes, and keep exact global cache work guarded until Annex B validation is explicit. |
...
| [ ] | Backlog | Dense array fast paths | Runtime architecture / performance | Continue guarded packed, holey, and sparse array fast paths now that storage has a separate owner. | Read/copy operations, default-attribute reverse, and default-attribute shift/unshift have initial packed paths. Continue reducing measured array debt in `concat`, `slice`, `includes`, `indexOf`, `lastIndexOf`, `join`, `reverse`, `shift`, and `unshift` with prototype-index guards, descriptor-aware mutation paths, bytecode/native-loop dispatch, and explicit materialization points. Preserve fallback semantics when holes, sparse indices, descriptors, or prototype index properties can affect reads or writes. |
...
| [ ] | Backlog | Broader per-site inline caches | Runtime architecture / performance | Extend the static member-read, static own-write, and `in` occurrence caches into call, function/native dispatch, computed-property, descriptor-aware write, and bytecode property access paths. | Static member reads, side-effect-free computed literal reads, static own-property writes, and `in` presence checks now have checked per-occurrence slots. Next cache shape id, slot offset, receiver/prototype guards, and descriptor facts for the remaining access kinds. Keep fallback paths correct, avoid cache churn across receiver shapes, and keep cache storage checked through newtype indices. |
| [ ] | Backlog | Compact values and heap strings | Runtime architecture / memory | Reduce scattered allocations and cloned strings without introducing unsafe allocator work first. | `StringId`, `JsString`, `StringHeap`, parser literal heap strings, the host return boundary, selected heap-backed runtime string paths, and VM string resource accounting now exist. Next migrate string wrapper/object storage, context-aware diagnostics, string index-property results, and other remaining `Value::String(String)` producers toward heap-backed values; then compact object/function/string handles, add Vec-backed heaps and free lists, box immutable constants, and defer NaN boxing unless safer structural changes are exhausted. |
...
changing the user global frame. First global cache misses can build guarded
locations directly from compiled `GlobalSlot` operands, active declaration cache
seeding can use compiled slots before falling back to runtime location search,
compound/update writes reuse the already resolved binding cell, and assignment,
compound assignment, `delete identifier`, and constructor lookup paths check for
an existing static binding cell before falling back to lazy builtin
materialization. That is still not the final design. The next slot work should
add true direct slot operands for the remaining read/write/eval paths, then
leave name lookup for dynamic or fallback paths only. Keep exact global
read/write caches guarded until Annex B shadowing behavior is explicit.
...
Holey arrays need explicit missing-element semantics: direct storage reads are
valid only when the relevant prototype-chain range cannot provide indexed
properties. Sparse arrays, descriptor-modified arrays, arrays affected by
prototype index properties, and non-contiguous packed storage for slice-style
operations must fall back to the generic object/property path or materialize
explicitly.
