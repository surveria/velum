...
| Shapes / hidden classes | Partial foundation. Ordinary object and function properties are atom-keyed, slot-backed, vector-indexed, ordinary object named-property lookups resolve slots through VM-owned `ShapeId` metadata, shapes include data-property descriptor attributes, cacheable named-property lookup snapshots exist for object/prototype reads and `in`, compiled static object-property reads can reuse guarded snapshots, and a VM-local prototype lookup version guards structural chain changes. | Add compiler-assigned property operands and move cache storage from static names to true per-occurrence interpreter or bytecode sites. |
| Inline caches | Partial bridge. Compiled static object-property reads can reuse monomorphic cacheable lookup snapshots through the static-name cache, guarded by receiver identity, receiver shape, and prototype lookup version. This is not a final per-site inline cache: repeated uses of the same static property name on different receivers can still overwrite the cache slot. | Add per-occurrence interpreter or bytecode cache slots for property loads, stores, calls, and `in` lookups. Keep shape, receiver, descriptor, and prototype guards explicit with generic fallback for stale or uncacheable paths. |
...
| [ ] | Backlog | Per-site inline caches | Runtime architecture / performance | Extend the static property-read cache into true per-occurrence interpreter or bytecode sites for property, call, store, and `in` access paths. | Cache shape id, slot offset, receiver/prototype guards, and descriptor facts where needed. Avoid static-name cache churn when the same property name appears on different receivers, keep fallback paths correct, and keep cache storage checked through newtype indices. |
...
The current static-name cache for compiled `obj.foo` reads is only an early
bridge: it proves the shape/prototype snapshot and fallback model, but the
long-term cache owner should be the property access occurrence or bytecode
operand.
