...
| Shapes / hidden classes | Partial foundation. Ordinary object and function properties are atom-keyed, slot-backed, vector-indexed, ordinary object named-property lookups resolve slots through VM-owned `ShapeId` metadata, shapes include data-property descriptor attributes, cacheable named-property lookup snapshots exist for object/prototype reads and `in`, compiled static object-property reads, side-effect-free computed literal property reads, and `in` presence checks can reuse guarded per-occurrence snapshots, and a VM-local prototype lookup version guards structural chain changes. | Add compiler-assigned property operands for the remaining dynamic property forms and extend guarded site caches beyond reads/presence checks. |
| Inline caches | Partial. Compiled static member reads, side-effect-free computed literal member reads, method-call references, and `in` presence checks now have per-occurrence cache slots instead of sharing lookup snapshots by static property name. Cached snapshots are still monomorphic and guarded by receiver identity, receiver shape, and prototype lookup version. | Extend per-occurrence cache slots to stores, broader static/computed calls, function/native dispatch, non-literal computed property keys, and bytecode property operands. Keep shape, receiver, descriptor, and prototype guards explicit with generic fallback for stale or uncacheable paths. |
