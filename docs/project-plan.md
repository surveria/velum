...
| Built-in intrinsics | Partial. Constructors and prototypes are lazily materialized per VM. Function and native-function `length`, `name`, and `prototype` paths now use typed property-kind dispatch before descriptor handling. | Separate immutable intrinsic metadata from mutable JS objects, pre-resolve atoms/shapes, and add direct native-call paths for common built-ins. |
