...
| Shapes / hidden classes | Partial foundation. Ordinary object and function properties are atom-keyed, slot-backed, vector-indexed, ordinary object named-property lookups resolve slots through VM-owned `ShapeId` metadata, shapes include data-property descriptor attributes, cacheable named-property lookup snapshots exist for object/prototype reads and `in`, compiled static object-property reads can reuse guarded per-occurrence snapshots, and a VM-local prototype lookup version guards structural chain changes. | Add compiler-assigned property operands for the remaining property forms and extend guarded site caches beyond static reads. |
| Inline caches | Partial. Compiled static member reads now have per-occurrence cache slots instead of sharing lookup snapshots by static property name. Cached snapshots are still monomorphic and guarded by receiver identity, receiver shape, and prototype lookup version, and method call references use the same static member read path. | Extend per-occurrence cache slots to `in`, stores, computed/static calls, function/native dispatch, and bytecode property operands. Keep shape, receiver, descriptor, and prototype guards explicit with generic fallback for stale or uncacheable paths. |
...
| [ ] | Backlog | Broader per-site inline caches | Runtime architecture / performance | Extend the static member-read occurrence cache into `in`, store, call, function/native dispatch, computed-property, and bytecode property access paths. | Static member reads now have checked per-occurrence slots. Next cache shape id, slot offset, receiver/prototype guards, and descriptor facts for the remaining access kinds. Keep fallback paths correct, avoid cache churn across receiver shapes, and keep cache storage checked through newtype indices. |
