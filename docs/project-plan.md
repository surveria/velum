...
| Slot-based bindings | Partial. `BindingScope` stores cells in a `Vec` behind an atom-to-slot map, the AST separates parser-interned name text from per-occurrence `StaticBinding` operands, compiled scripts now build a scope-aware binding layout with `GlobalSlot`, `LocalSlot`, and `UpvalueSlot` metadata, and compiled-script evaluation still keeps a guarded script-local `StaticBindingId -> BindingSlot` cache for current runtime semantics. | Use the compiled binding layout to seed direct local/global/upvalue access, then remove ordinary runtime shadowing scans from hot binding reads and writes. |
