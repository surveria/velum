...
| Shapes / hidden classes | Partial foundation. Ordinary object and function properties are atom-keyed, slot-backed, vector-indexed, and ordinary objects now carry VM-owned `ShapeId` layout metadata. | Add shared property offsets, prototype-version guards, descriptor-aware transitions, and cacheable lookup APIs for objects and prototypes. |
