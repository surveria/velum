...
| Shapes / hidden classes | Partial foundation. Ordinary object and function properties are atom-keyed, slot-backed, vector-indexed, and ordinary object named-property lookups now resolve slots through VM-owned `ShapeId` metadata. | Add prototype-version guards, descriptor-aware transitions, and cacheable lookup APIs for objects and prototypes. |
