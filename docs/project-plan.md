...
| Slot-based bindings | Partial. `BindingScope` stores cells in a `Vec` behind an atom-to-slot map, the AST separates parser-interned name text from per-occurrence `StaticBinding` operands, and compiled-script evaluation now has a script-local checked `StaticBindingId -> BindingSlot` cache for repeated global/local binding access. | Add compile-time scope layout with `LocalSlot`, `GlobalSlot`, and `UpvalueSlot`, then make ordinary local access index-based without runtime shadowing scans. |
