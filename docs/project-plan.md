...
| Built-in intrinsics | Partial. Constructors and prototypes are lazily materialized per VM. Function and native-function `length`, `name`, and `prototype` paths now use typed property-kind dispatch before descriptor handling, and immutable default `length`/`name` descriptors are stored on `FunctionProperties` instead of rebuilt from function name/arity metadata on every descriptor or value read. | Separate the remaining immutable intrinsic metadata from mutable JS objects, pre-resolve atoms/shapes, avoid synthesizing prototype descriptors on hot paths, and add direct native-call paths for common built-ins. |
