...
| Slot-based bindings | Partial. `BindingScope` stores cells in a `Vec` behind an atom-to-slot map, the AST separates parser-interned name text from per-occurrence `StaticBinding` operands, compiled scripts build a scope-aware binding layout with `GlobalSlot`, `LocalSlot`, and `UpvalueSlot` metadata, and compiled evaluation uses that layout to seed all matching static binding occurrence caches once a runtime binding location is known. Runtime access still validates cached locations and falls back to name resolution. | Build runtime frames and closure cells directly from the compiled layout so reads, writes, compound assignments, updates, and constructor lookups can use checked slot operands without ordinary shadowing scans. |
| Atom interner | Partial. VM-local atoms cover lexical/global binding names and the atom table now uses a sorted vector index instead of a tree map; static AST names are typed and deduplicated per parsed script so later property and binding operands can attach atom/slot metadata without changing parser surfaces again. | Extend atoms to property names, built-in names, function names, string literal keys, shapes, prototype lookup, and diagnostics so hot paths do not fall back to `String`. |
...
| Dense arrays | Partial foundation. Arrays now use a dedicated `ArrayStorage` with packed and holey dense elements plus sparse index keys, but hot methods still mostly use generic property semantics. | Add guarded fast paths for scan, copy, concat, join, reverse, shift, and unshift, with fallback for holes, sparse indices, descriptors, and prototype index properties. |
...
| [ ] | Backlog | Compiler-assigned slots and upvalues | Runtime architecture / performance | Replace repeated runtime name lookup for locals, globals, and captured variables with checked slot indices. | The compiled `BindingLayout` now exists and seeds matching runtime binding-cache occurrences after a safe runtime location is known. The next step is using that metadata to build runtime frames and closure cells directly, then making reads, writes, compound assignments, updates, and constructor lookups use checked slot operands instead of runtime name resolution. Do not assume current runtime insertion order matches compiled slot order; create frames from the layout first. |
...
`UpvalueSlot` operands and can seed matching occurrence caches after runtime
locations are known. That is still not the final design. The next slot branch
should create runtime frames and closure cells from the compiled layout itself,
then leave name lookup for dynamic or fallback paths only.
