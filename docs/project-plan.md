...
and compound/update writes reuse the already resolved binding cell. That is
still not the final design. The next slot work should add runtime frame
metadata for local scope ids, closure/upvalue frame cells, and direct slot
operands for all reads, writes, compound assignments, updates, and constructor
lookups, then leave name lookup for dynamic or fallback paths only. Keep exact
global read/write caches guarded until Annex B shadowing behavior and builtin
fallback validation are explicit.
