...
| Shapes / hidden classes | Partial foundation. Ordinary object and function properties are atom-keyed, slot-backed, and vector-indexed per owner, but layouts are not shared yet. | Introduce `ShapeId`, shared shape tables, slot offsets, and explicit transitions for ordinary objects and prototypes. |
