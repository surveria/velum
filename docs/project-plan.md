...
| Slot-based bindings | Partial. `BindingScope` stores cells in a `Vec` behind an atom-to-slot map, and the AST now carries parser-interned static names through a typed `StaticName` operand instead of raw `String` fields. | Add compile-time scope layout with `LocalSlot`, `GlobalSlot`, and `UpvalueSlot`, then make ordinary local access index-based. |
