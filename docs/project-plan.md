...
| Memory layout | Partial indexed handles for objects, functions, native functions, and host functions. A VM-owned `StringHeap` now provides `StringId` and heap-backed `JsString` handles for selected runtime-generated strings, including parser string/template literals, `typeof`, concatenation results, `String(...)`, native function names, JSON strings, `Object.keys` results, and `for...in` keys. `Value` still supports owned strings for host returns, context-free diagnostics, and remaining externalized values. | Extend `StringId` into host conversion, object/string wrapper paths, diagnostics where context is available, compact handles, Vec-backed heaps, free lists, boxed immutable constants, and captured-variable cells only where closures need them. |
...
| [ ] | Backlog | Compact values and heap strings | Runtime architecture / memory | Reduce scattered allocations and cloned strings without introducing unsafe allocator work first. | `StringId`, `JsString`, `StringHeap`, parser literal heap strings, selected heap-backed runtime string paths, and VM string resource accounting now exist. Next migrate host-returned strings, string wrapper/object storage, context-aware diagnostics, string index-property results, and other remaining `Value::String(String)` producers toward heap-backed values; then compact object/function/string handles, add Vec-backed heaps and free lists, box immutable constants, and defer NaN boxing unless safer structural changes are exhausted. |
