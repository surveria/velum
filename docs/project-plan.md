...
| [ ] | Backlog | Standard errors and function intrinsic semantics tranche | Compatibility / embedding API | Improve standard error objects and complete configurable function intrinsic behavior. | Split from the function descriptor branch so descriptor support could land narrowly. Cover standard error constructors/prototypes, error descriptors, and full redefinition/delete behavior for configurable function `name` and `length` before enabling upstream `built-ins/Function` rows. |
