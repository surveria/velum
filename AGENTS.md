...
- `scripts/with-host-lock.sh` coordinates every repository worktree on the machine through `flock`: tests use a shared lock, benchmarks use an exclusive lock, and `/tmp/rsqjs-host-performance.lock.owner` records diagnostic PID, start time, working directory, and command while an exclusive run owns the host.
