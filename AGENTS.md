...
- A successful merge into `main` is the completion boundary for the feature task. Do not wait for the post-merge `publish` / `publish-report` stage, benchmark report generation, chart generation, or the report-only commit before closing the task and selecting the next one. Those jobs are best-effort follow-up work: if they fail, leave them for the next merged PR to regenerate unless the user explicitly opens a separate report-publication recovery task. Do not rerun or repair post-merge publication as part of an ordinary feature task.
...
- The engine, interactive CLI, and test/benchmark runner live in this repository but remain separate crates:
  - `velum` (the root crate): the embeddable engine library plus the `velum` smoke CLI. It is a standalone, dependency-light crate — only what the engine itself needs. Do not add terminal, reporting, benchmarking, or reference-engine dependencies here. It builds and tests on its own, without the interactive CLI or runner.
  - `cli/`: the `velum-cli` nested workspace and interactive `velum` binary. It owns terminal and line-editing dependencies, depends on the engine only through `velum = { path = ".." }`, and must not move terminal dependencies into the embeddable engine crate.
  - `runner/`: the `velum-test-runner` nested workspace. It owns heavier dependencies such as `tabled`, `plotters`, `image`, `serde`, `rquickjs`, and report aggregation code. It depends on the engine only through `velum = { path = ".." }` and the public API.
- To build or run the interactive CLI by hand, use `--manifest-path cli/Cargo.toml`.
...
- Keep the root, `cli/`, and `runner/` `Cargo.lock` files tracked. Update them when the dependency graph or local package metadata actually changes, but do not create lockfile churn for synthetic per-PR version bumps.
- Bump package versions only in explicit release/version pull requests. The three crates version independently; use semantic versioning, update the corresponding lockfile entries, state the new versions and reason in the PR description, run the full gate, and tag the merged release commit when a release tag is needed.
...
- `velum` is an embeddable Rust library first. The CLI, test runner, and scripts are support surfaces for smoke testing, differential checks, and benchmark orchestration.
...
- `scripts/check-fast.sh` is the local iteration gate. It runs formatting, clippy, tests, and docs for the engine without materializing external corpora or generating benchmark reports. Set `VELUM_FAST_CLI=1` for the interactive CLI and `VELUM_FAST_RUNNER=1` for the runner. Correctness runs do not acquire the benchmark host lock and may overlap freely.
...
- `scripts/check-correctness.sh` is the required ready-PR and merge-queue gate. It runs formatting, strict clippy, tests, docs, the active fixture registries, the QuickJS differential corpus, and the complete pinned Test262 corpus without project or JetStream benchmarks. Test262 paths run on `VELUM_TEST_JOBS` workers, defaulting to 30 in GitHub Actions and four for an explicit local full run. Lost known passes fail the gate; new passes are accepted into an exact-tree artifact candidate that the trusted post-merge publisher commits with the canonical report.
...
- `scripts/run-jetstream.sh` is the standalone JetStream report entrypoint. Preparation and compilation remain unlocked; the runner acquires the exclusive host lock only around the measured suite. It does not repeat correctness, Test262, differential, or project benchmark work. Use `VELUM_JETSTREAM_FILTER` for exact ids or explicit trailing-star prefixes during local investigation. Normal runs use the committed content-addressed QuickJS baseline without live fallback; a full `VELUM_JETSTREAM_QUICKJS_BASELINE=refresh` run is an explicit maintenance operation, not routine validation.
- The Rust runner coordinates every repository worktree and containerized GitHub Runner through `flock`, acquiring the exclusive host lock only around measured project or JetStream execution. The sibling `.owner` file records diagnostic PID, UID, host, start time, working directory, and benchmark label while the measured run owns the host. Local worktrees use `/run/lock/velum/host-performance.lock`; runner containers receive that host directory at `/host/velum-lock` and CI sets `VELUM_HOST_LOCK_PATH` accordingly. Do not use container-private `/tmp` or `/run` paths. `scripts/with-host-lock.sh` remains only for explicit whole-command maintenance locking.
