...
- After a PR is merged, GitHub Actions checks out the exact merge commit, collects the project benchmark report under the exclusive host lock, uploads the tree-keyed performance artifact, stores the tested source commit on the hidden `refs/rsqjs/ci-tested-sources` archive ref, copies exactly one compact YAML summary plus its derived Markdown report into `reports/test-runs/`, regenerates `reports/benchmark-rollup.md` and `reports/benchmark-summary.jpg`, and pushes one report-only commit to `main`. Full per-case YAML remains in the CI artifact. JetStream is not part of this per-merge path.
