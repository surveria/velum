...
- After a PR is merged, GitHub Actions checks out the exact merge commit and runs only the prepared project benchmark set under the exclusive host lock. The publisher downloads the exact-tree correctness and performance YAML artifacts, rejects missing, stale, duplicate-metadata, or cross-tree inputs, composes their typed models, stores the tested source commit on the hidden `refs/rsqjs/ci-tested-sources` archive ref, copies one compact YAML summary plus its derived Markdown report into `reports/test-runs/`, regenerates `reports/benchmark-rollup.md` and `reports/benchmark-summary.jpg`, and pushes one report-only commit to `main`. Full per-case YAML and TSV remain in the CI artifact. JetStream is not part of this per-merge path.
...
- Canonical tracked test reports belong under `reports/test-runs/` as a compact schema-versioned YAML source plus a derived Markdown file with the same sortable UTC timestamp. Full per-case YAML and TSV stay in the tree-keyed CI artifact instead of git. Canonical tracked JetStream shell reports belong under `reports/jetstream-runs/`. Per-merge CI publishes project benchmark history and regenerates the tracked rollup and chart from YAML; JetStream is an explicit full/manual lane and is published only when that lane supplies a matching artifact. Generate one manually only by setting `RSQJS_TRACKED_REPORT=1` or an explicit `RSQJS_TEST_REPORT_PATH`.
