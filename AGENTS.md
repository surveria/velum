...
- After a PR is merged, GitHub Actions publishes the canonical report: it downloads the report artifact for the tested tree, stores the tested source commit on the single `ci-tested-sources` archive branch so report commit SHAs remain fetchable, copies exactly one test report into `reports/test-runs/`, copies the matching JetStream shell report into `reports/jetstream-runs/` when that artifact is present, regenerates `reports/benchmark-rollup.md` and `reports/benchmark-summary.jpg`, and pushes one report-only commit to `main`.
...
- Ordinary feature PRs must not commit generated full test reports. Full reports generated by CI live under `target/rsqjs-reports/`, including `target/rsqjs-reports/jetstream-runs/` for JetStream shell reports, and are uploaded as artifacts named by the tested tree SHA.
- Canonical tracked test reports belong under `reports/test-runs/`. Canonical tracked JetStream shell reports belong under `reports/jetstream-runs/`. Each tracked report must be a separate Markdown file with a sortable UTC timestamp suffix. After merge, CI publishes the canonical reports from the already-tested artifact and regenerates the tracked rollup and chart. Generate one manually only by setting `RSQJS_TRACKED_REPORT=1` or an explicit `RSQJS_TEST_REPORT_PATH`.
...
- Future benchmark reports should follow the same pattern: one command, Rust-owned execution, CI artifacts for ordinary PRs, post-merge tracked canonical reports, and clear comparison against QuickJS where possible. External benchmark families such as JetStream shell workloads should use their own report directory while still feeding the shared rollup and summary chart.
