...
- `scripts/with-host-lock.sh` coordinates every repository worktree and the systemd GitHub Runner service through `flock`: tests use a shared lock, benchmarks use an exclusive lock, and `/run/lock/rsqjs/host-performance.lock.owner` records diagnostic PID, UID, host, start time, working directory, and command while an exclusive run owns the host. Do not move this coordination file into `/tmp`; the service runner has a private `/tmp` namespace.
