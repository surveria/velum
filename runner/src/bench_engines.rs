...
use velum::{CompiledScript, Context, RuntimeLimits};
...
/// library defaults.
pub struct VelumEngine;

impl BenchEngine for VelumEngine {
    fn label(&self) -> &'static str {
        "velum"
    }

    fn eval(&self, source: &str) -> anyhow::Result<()> {
        let runtime = velum::Runtime::with_limits(BENCH_RUNTIME_LIMITS);
        let mut context = runtime.context();
        let value = context
            .eval(source)
            .map_err(|error| anyhow::anyhow!("Velum eval failed: {error}"))?;
        black_box(value);
    ...
    fn eval_with_host_image(&self, source: &str, byte_len: usize) -> anyhow::Result<()> {
        ...
        context
            ...
            .map_err(|error| anyhow::anyhow!("Velum host image setup failed: {error}"))?;
        let value = context
            .eval(source)
            .map_err(|error| anyhow::anyhow!("Velum eval failed: {error}"))?;
        black_box(value);
    ...
    fn prepare(
        ...
        let source_script = runtime
            .compile(source)
            .map_err(|error| anyhow::anyhow!("Velum benchmark source compile failed: {error}"))?;
...
impl PreparedBenchSession for VelumPreparedSession {
    fn run(&mut self) -> anyhow::Result<BenchmarkChecksum> {
        let value = self
            ...
            .map_err(|error| anyhow::anyhow!("Velum prepared benchmark run failed: {error}"))?;
        BenchmarkChecksum::from_velum(value)
    }
    ...
    fn verify(&mut self) -> anyhow::Result<BenchmarkChecksum> {
        let value = self
            ...
            .map_err(|error| anyhow::anyhow!("Velum prepared benchmark verify failed: {error}"))?;
        BenchmarkChecksum::from_velum(value)
    }
}
...
fn compile_call(
    runtime: &velum::Runtime,
    source: &str,
    phase: &str,
) -> anyhow::Result<CompiledScript> {
    runtime
        .compile(source)
        .map_err(|error| anyhow::anyhow!("Velum benchmark {phase} call compile failed: {error}"))
}

fn install_velum_input(context: &mut Context, input: BenchmarkInput) -> anyhow::Result<()> {
    match input {
...
/// Velum numbers without a cross-engine ratio.
...
/// Minimal shim so benchmark scripts that call the Velum host `print` also
