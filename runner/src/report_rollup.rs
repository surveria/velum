...
const REPORT_PREFIX: &str = "velum-test-report-";
...
fn write_rollup(rollup: &RollupReport) -> anyhow::Result<()> {
    ...
    write_svg_chart(
        &rollup.records,
        &rollup.timeline,
        &rollup.outputs.summary_chart_light_svg,
        ChartTheme::Light,
    )?;
    write_svg_chart(
        &rollup.records,
        &rollup.timeline,
        &rollup.outputs.summary_chart_dark_svg,
        ChartTheme::Dark,
    )
}
...
fn render_markdown(records: &[ReportRecord]) -> String {
    let mut lines = vec![
        "# Velum Benchmark Rollup".to_owned(),
        String::new(),
        "Generated by `velum-test-runner` from tracked test reports.".to_owned(),
        String::new(),
