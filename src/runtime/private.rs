...
impl PrivateNameId {
    const fn new(environment: u64, index: u32) -> Self {
...
impl PrivateEnvironment {
    ...
    pub(in crate::runtime) fn resolve(&self, name: &str) -> Result<PrivateNameId> {
            ...
            .position(|candidate| candidate.as_str() == name)
        {
            ...
            return Ok(PrivateNameId::new(self.id, index));
        }
    ...
    pub(in crate::runtime) fn own_name(&self, index: u32) -> Result<PrivateNameId> {
        ...
        Ok(PrivateNameId::new(self.id, index))
    }
...
impl Context {
    pub(in crate::runtime) fn begin_private_environment(
        ...
        let id = self.next_private_environment_id;
        self.next_private_environment_id = id
            .checked_add(1)
            .ok_or_else(|| Error::limit("private environment id overflowed"))?;
        let parent = state.private_environment();
        state
            .replace_private_environment(Some(Rc::new(PrivateEnvironment::new(id, names, parent))));
        Ok(())
    }
    ...
    pub(in crate::runtime) fn resolve_private_name(
        ...
        let Some(environment) = &self.active_private_environment else {
    ...
    pub(in crate::runtime) fn leave_private_environment(
        ...
        state.replace_private_environment(environment.parent());
        Ok(())
    }
