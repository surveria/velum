...
use super::{
    ...
    ASYNC_ITERATOR_DISPOSE_NAME, ASYNC_ITERATOR_SELF_NAME, BIGINT_AS_INT_N_NAME,
    BIGINT_AS_UINT_N_NAME, BIGINT_FUNCTION_LENGTH, BIGINT_NAME, BOOLEAN_FUNCTION_LENGTH,
    BOOLEAN_NAME, BOUND_FUNCTION_LENGTH, BOUND_FUNCTION_NAME, ERROR_IS_ERROR_LENGTH,
    ERROR_IS_ERROR_NAME, ERROR_PROTOTYPE_TO_STRING_LENGTH, ERROR_PROTOTYPE_TO_STRING_NAME,
    ERROR_STACK_GETTER_LENGTH, ERROR_STACK_GETTER_NAME, ERROR_STACK_SETTER_LENGTH,
    ERROR_STACK_SETTER_NAME, EVAL_FUNCTION_LENGTH, EVAL_NAME, FUNCTION_FUNCTION_LENGTH,
    FUNCTION_NAME, FUNCTION_PROTOTYPE_APPLY_LENGTH, FUNCTION_PROTOTYPE_APPLY_NAME,
    FUNCTION_PROTOTYPE_BIND_LENGTH, FUNCTION_PROTOTYPE_BIND_NAME, FUNCTION_PROTOTYPE_CALL_LENGTH,
    FUNCTION_PROTOTYPE_CALL_NAME, FUNCTION_PROTOTYPE_HAS_INSTANCE_LENGTH,
    FUNCTION_PROTOTYPE_HAS_INSTANCE_NAME, FUNCTION_PROTOTYPE_TO_STRING_LENGTH,
    FUNCTION_PROTOTYPE_TO_STRING_NAME, GENERATOR_FUNCTION_FUNCTION_LENGTH, GENERATOR_FUNCTION_NAME,
...
impl NativeFunctionKind {
    pub(super) const fn core_length(self) -> Option<f64> {
        ...
        match self {
            ...
            Self::AsyncIteratorDispose | Self::AsyncIteratorSelf => Some(0.0),
            ...
            Self::ShadowRealm(kind) => Some(kind.length()),
            Self::Eval => Some(EVAL_FUNCTION_LENGTH),
            Self::ErrorConstructor(name) => Some(name.constructor_length()),
            Self::ErrorIsError => Some(ERROR_IS_ERROR_LENGTH),
            Self::ErrorPrototypeToString => Some(ERROR_PROTOTYPE_TO_STRING_LENGTH),
            Self::ErrorStackGetter => Some(ERROR_STACK_GETTER_LENGTH),
            Self::ErrorStackSetter => Some(ERROR_STACK_SETTER_LENGTH),
            Self::Function => Some(FUNCTION_FUNCTION_LENGTH),
            Self::GeneratorFunction => Some(GENERATOR_FUNCTION_FUNCTION_LENGTH),
    ...
    pub(super) const fn core_name(self) -> Option<&'static str> {
        ...
        match self {
            ...
            Self::ShadowRealm(kind) => Some(kind.name()),
            Self::Eval => Some(EVAL_NAME),
            Self::ErrorConstructor(name) => Some(name.as_str()),
            Self::ErrorIsError => Some(ERROR_IS_ERROR_NAME),
            Self::ErrorPrototypeToString => Some(ERROR_PROTOTYPE_TO_STRING_NAME),
            Self::ErrorStackGetter => Some(ERROR_STACK_GETTER_NAME),
            Self::ErrorStackSetter => Some(ERROR_STACK_SETTER_NAME),
            Self::Function => Some(FUNCTION_NAME),
            Self::GeneratorFunction => Some(GENERATOR_FUNCTION_NAME),
