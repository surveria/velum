...
#[derive(Debug, Clone)]
pub(super) struct NativeFunctionRegistry {
...
impl NativeFunctionRegistry {
    ...
    pub(super) fn get(&self, kind: NativeFunctionKind) -> Option<NativeFunctionId> {
        self.slots.get(slot(kind).index()).copied().flatten()
    }

    pub(super) fn insert(&mut self, kind: NativeFunctionKind, id: NativeFunctionId) -> Result<()> {
        let slot = slot(kind);
        let entry = self
            .slots
            .get_mut(slot.index())
