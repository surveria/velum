...
fn finding_directory(root: &Path, finding: CaseFinding) -> PathBuf {
    match finding {
        ...
        CaseFinding::VelumResourceLimit => root.join("findings/velum-resource-limits"),
...
impl CaseClassification {
    ...
    pub const fn as_legacy_finding(self) -> Option<CaseFinding> {
        match self {
            ...
            Self::VelumResourceLimit => Some(CaseFinding::VelumResourceLimit),
...
fn create_layout(root: &Path) -> anyhow::Result<()> {
    for path in [
        ...
        &root.join("findings/velum-resource-limits"),
