...
<p align="center">
  <a href="reports/test-runs/velum-test-report-20260715T084223Z.md"><img alt="Test262: 102,578 of 102,578" src="https://img.shields.io/badge/Test262-102%2C578%20%2F%20102%2C578-2ea44f?style=for-the-badge"></a>
...
| Profiled runtime optimization | Parent/candidate; `QuickJS` checks for prepared JS | PR #724: 28 cases × 4 = 112 valid timing rows; 144 memory workers passed; 49 new regression cases | Mixed workloads -2.4% time; holdouts -0.9%; six selected `JetStream` cases -23.2%; focused `WeakMap` GC 1.08–2.69x faster | [Reviewed second tranche](docs/safe-runtime-performance.md#reviewed-runtime-results-2026-09-22); arithmetic sentinel +2.6% time; bounded host/cohort evidence, not a universal speedup |
| String admission optimization | Parent/candidate; paired `QuickJS` measurements | PR #725: 112 valid timing rows; 144 memory workers passed; 11 new regression cases | Base64 -32.4% time; tagcloud -15.3%; six selected `JetStream` cases -7.9%; embedding -4.6% | [Reviewed tradeoffs](docs/interpreter-gap-campaign.md#reviewed-results-2026-09-22): mixed workloads +0.9%, holdouts +1.3%, worst individual slowdown +4.4%; exact logical memory counters unchanged |
| Bytecode roots and GC re-entry | Parent/candidate; paired `QuickJS` measurements and explicit GC callbacks | PR #726 final matrix: 112 valid timing rows; 144 memory workers passed; 26 new regression cases | Mixed workloads -3.1% time; holdouts -2.3%; embedding and six selected `JetStream` cases -2.4%; fixed disposal/Promise lifetime gaps | [Reviewed experiment](docs/bytecode-root-performance.md#reviewed-results-2026-09-23): initial implementation rejected; worst final paired slowdown +0.2%; logical memory counters unchanged; host/cohort evidence only |
| Binding reads and eval recreation | Parent/candidate; paired `QuickJS` measurements | PR #727: 112 valid timing rows; 144 memory workers passed; 20 new regression cases | Mixed workloads -1.6% time; holdouts -2.0%; embedding -1.1%; selected `JetStream` -0.9%; fixed deleted eval-binding recreation | [Reviewed tradeoffs](docs/binding-read-performance.md#reviewed-results-2026-09-23): sentinels +0.6%, worst individual slowdown +2.4%; logical counters unchanged; two separate catch/eval repros remain unresolved |
| Separate `ThinLTO` + one codegen unit | Same-source, same-path ordinary / `ThinLTO` builds; no PGO | 24 valid holdout timing rows; saved `ThinLTO` binary passes all 102,578 Test262 variants and 99 `QuickJS` cases | Holdouts -7.2% time; output checksums match; whole runner -12.2% file size | [Reviewed compiler configuration](docs/safe-runtime-performance.md#separate-thinlto-configuration-experiment); one host/cohort; ordinary defaults unchanged |
| Controlled PGO experiment | Opt-in launcher; 6 training programs and 6 separate holdouts; ordinary/PGO builds | Fresh run: 24 valid timing rows; 144 memory workers; saved PGO binary passes all 102,578 Test262 variants and 99 `QuickJS` cases | Holdouts -29.7% time; all output checksums and logical memory counters match; whole runner -3.9% file size | [Reviewed repository launcher](docs/pgo-experiment.md#reviewed-repository-launcher-2026-09-22); one host/cohort, ordinary defaults unchanged; historical prototype results remain separate |
